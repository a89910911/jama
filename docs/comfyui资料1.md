...
在 **JamaAI**（本地 AI 短剧全流程创作工具）等项目生态中，本代理承载着底层生成引擎的统一 API 基座角色，为从剧本到成片的完整链路提供稳定、可扩展的推理调度能力。
...
- **统一生成接口**：`JamaAI` 通过标准 OpenAI API 调用本代理，无需关心底层 ComfyUI 工作流细节。
