...
- 项目模块提供“项目管线”入口，支持单项目覆盖。
- 项目任务优先使用项目管线，没有项目覆盖时自动继承系统管线。
...
3. 列表默认显示所有可用于该项目的提示词，以及当前来源：
   - `继承系统`
   - `项目自定义`
4. 点击“创建项目覆盖”或“编辑项目管线”。
5. 编辑器初始内容为当前系统管线副本。
6. 保存后只影响当前项目。
7. 点击“恢复使用系统管线”时，删除项目级记录，立即恢复继承系统值。
...
禁止只使用“系统管线”“通用规则”“合同”“处理模板”等无法说明业务对象和结果的名称。最终 94 个提示词的用户可读名称必须完整、唯一，并与正文实际作用一致。新增提示词没有配置用户可读名称、名称类型与消息角色不一致、名称重复或正文为空时，目录初始化必须报错。
...
- 保存后创建或更新项目级记录。
- 项目级内容不能为空；需要继承系统时使用“恢复使用系统管线”删除项目记录。
- 系统管线后续修改时：
  - 仍在继承的项目立即使用新系统内容。
  - 已有项目覆盖的项目不受影响。
...
|---|---|---|---|
| `story.generation.system` | 故事生成系统管线 | system | `story_generation` |
...
- 所有静态 system/user/suffix/format/image/video/negative 模板都能在系统管线列表中找到。
- 最终提示词预览与实际发送内容一致。
- 当前 9 条自定义覆盖数据可以无损迁移。
...
7. **系统级恢复出厂默认**  
   数据库同时保存当前内容 `content`、出厂内容 `seed_content` 和出厂版本 `seed_version`。系统管线只能由用户主动恢复出厂默认；软件升级不得自动覆盖用户已修改内容。新版默认值与当前内容不同时，向用户提示并允许查看差异、主动采用。
