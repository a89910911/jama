...
- 项目管线对普通创作者可见，但“高风险”模板不建议非专业人员修改。
...
页面右上角“明色/暗色”可切换主题；侧边竖排悬浮的“素材生成进度”用于跨页面查看后台任务，并支持拖动到左右边缘。首页“微信我”是联系支持入口。
...
系统提供火山、Agnes、fal.ai、Venice、MediaBridge、通义等一键预设：
...
顶部还可切换分集、进入画布模式、打开 Codex AI 助手、项目管线、AI 记录和 AI 配置。
...
![项目管线](user-manual-assets/09-project-prompts.png)
...
| JSON 解析失败 | 提示词模板输出格式被改坏 | 恢复系统管线或项目继承 |
| 人物不一致 | 主图/身份锚点未统一，参考图过多或冲突 | 固定主图，完善身份锚点并清理冲突参考图 |
...
固定同一角色主图；完善身份锚点；减少冲突参考图；保持发型和服装描述稳定；连续镜头可使用首尾帧和尾帧衔接。
