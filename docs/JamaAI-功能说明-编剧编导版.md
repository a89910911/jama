...
普通账号可以进入项目、编辑剧本、管理资源、生成分镜和媒体、使用画布、项目管线、Codex AI 助手和项目 AI 记录。
...
| 项目管线 | 项目级覆盖、分类检索、业务场景模型映射 | 高级创作者 |
...
*图 4　角色卡：人物说明、主图、生成/上传、模板、素材库和关联分镜集中展示。*
...
项目管线允许当前项目覆盖系统默认提示词。当前系统按剧本、资产、分镜、视频等分类管理约百项模板，并标明主模板/条件子模板、System/User 消息、业务场景、变量和风险等级。
...
![项目管线管理](user-manual-assets/09-project-prompts.png)
...
页面侧边的竖排悬浮“素材生成进度”跨页面显示后台任务数量和状态。浮签可上下拖动并吸附到左右边缘，系统会记住最后位置；离开当前制作区后仍可查看任务，不需要停留在按钮旁等待。
...
每类服务只能有一条默认配置。配置支持添加、编辑、测试、设为默认、删除、批量删除、JSON 导入/导出，以及火山、Agnes、fal.ai、Venice、MediaBridge、通义等一键预设。非默认行可在列表中直接点击“设为默认”，切换只影响同一种服务类型，不会改变其他类型的默认项。
...
- **MediaBridge 资产管理**：查询、上传本地文件、从 URL 导入、预览、下载、查看详情和删除 MediaBridge 账号内的图片/视频/音频素材。
...
| 管理员 | AI 服务、默认模型、并发、账号、系统管线、厂商资产 |
...
5. 首尾帧、全能多图和厂商资产属于模型相关能力，只有相应服务商和模型支持时才生效。
6. 高并发容易触发 429；出现连续失败时先降低并发，再查看 AI 记录。
7. 分镜视频不完整时不要直接合成整集，否则会出现缺镜或静音段。
8. 项目管线的高风险模板由管理员或熟悉接口协议的人维护。
