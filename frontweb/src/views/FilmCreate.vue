<template>
  <div class="film-create" :class="{ 'sidebar-collapsed': navCollapsed }">
    ...
    <main class="main">
      ...
      <section class="section card resource-panel">
        <div class="collapse-header" @click="resourcePanelCollapsed = !resourcePanelCollapsed">
          <div class="resource-header-title">
            ...
            <span v-if="assetBatchGenerating" class="resource-batch-progress">
              {{ assetBatchProgress.current }}/{{ assetBatchProgress.total }}
              <template v-if="assetBatchProgress.failed">（失败 {{ assetBatchProgress.failed }}）</template>
            </span>
          </div>
          <div class="resource-header-right">
            <div class="resource-batch-actions" @click.stop>
              <el-tooltip content="生成当前集尚无图片的角色、场景和道具" placement="top">
                <el-button
                  type="primary"
                  size="small"
                  :loading="assetBatchGenerating && assetBatchGenerationScope === 'all'"
                  :disabled="!currentEpisodeId || pipelineRunning || assetBatchGenerating"
                  @click="startBatchAssetGeneration('all')"
                >
                  <el-icon v-if="!assetBatchGenerating"><MagicStick /></el-icon>
                  批量生成
                </el-button>
              </el-tooltip>
              <el-tooltip content="按资产名称打包；图片为 JPG，视频为 MP4" placement="top">
                <el-button
                  size="small"
                  :loading="assetBatchExportingScope === 'all'"
                  :disabled="!currentEpisodeId || !!assetBatchExportingScope"
                  @click="batchExportAssets('all')"
                >
                  <el-icon v-if="assetBatchExportingScope !== 'all'"><Download /></el-icon>
                  批量导出
                </el-button>
              </el-tooltip>
            </div>
          ...
          <div id="anchor-characters" class="resource-block card">
            ...
            <div v-show="!charactersBlockCollapsed" class="resource-block-body">
              <div class="asset-actions">
                ...
                <el-button size="small" @click="showCharLibrary = true">角色模板与复用</el-button>
                <el-button
                  type="success"
                  plain
                  size="small"
                  :loading="assetBatchGenerating && assetBatchGenerationScope === 'characters'"
                  :disabled="!currentEpisodeId || pipelineRunning || assetBatchGenerating || characters.length === 0"
                  @click="startBatchAssetGeneration('characters')"
                >
                  批量生成角色
                </el-button>
                <el-button
                  plain
                  size="small"
                  :loading="assetBatchExportingScope === 'characters'"
                  :disabled="!currentEpisodeId || !!assetBatchExportingScope || characters.length === 0"
                  @click="batchExportAssets('characters')"
                >
                  <el-icon v-if="assetBatchExportingScope !== 'characters'"><Download /></el-icon>
                  批量导出角色
                </el-button>
              </div>
          ...
          <div id="anchor-props" class="resource-block card">
            ...
            <div v-show="!propsBlockCollapsed" class="resource-block-body">
              <div class="asset-actions">
                ...
                <el-button size="small" @click="showPropLibrary = true">道具模板与复用</el-button>
                <el-button
                  type="success"
                  plain
                  size="small"
                  :loading="assetBatchGenerating && assetBatchGenerationScope === 'props'"
                  :disabled="!currentEpisodeId || pipelineRunning || assetBatchGenerating || props.length === 0"
                  @click="startBatchAssetGeneration('props')"
                >
                  批量生成道具
                </el-button>
                <el-button
                  plain
                  size="small"
                  :loading="assetBatchExportingScope === 'props'"
                  :disabled="!currentEpisodeId || !!assetBatchExportingScope || props.length === 0"
                  @click="batchExportAssets('props')"
                >
                  <el-icon v-if="assetBatchExportingScope !== 'props'"><Download /></el-icon>
                  批量导出道具
                </el-button>
              </div>
          ...
          <div id="anchor-scenes" class="resource-block card">
            ...
            <div v-show="!scenesBlockCollapsed" class="resource-block-body">
              <div class="asset-actions">
                ...
                <el-button size="small" @click="showSceneLibrary = true">场景模板与复用</el-button>
                <el-button
                  type="success"
                  plain
                  size="small"
                  :loading="assetBatchGenerating && assetBatchGenerationScope === 'scenes'"
                  :disabled="!currentEpisodeId || pipelineRunning || assetBatchGenerating || scenes.length === 0"
                  @click="startBatchAssetGeneration('scenes')"
                >
                  批量生成场景
                </el-button>
                <el-button
                  plain
                  size="small"
                  :loading="assetBatchExportingScope === 'scenes'"
                  :disabled="!currentEpisodeId || !!assetBatchExportingScope || scenes.length === 0"
                  @click="batchExportAssets('scenes')"
                >
                  <el-icon v-if="assetBatchExportingScope !== 'scenes'"><Download /></el-icon>
                  批量导出场景
                </el-button>
              </div>
      ...
      <section id="anchor-storyboard" class="section card">
        ...
        <div class="sb-config-row sb-narration-export-row" style="margin-top:10px;flex-wrap:wrap;align-items:center;gap:12px">
          <span class="sb-config-scope-hint">以下为全局默认；每个「分镜配置」中的设置优先。</span>
          ...
          <template v-if="storyboards.length > 0">
            <div class="sb-batch-right">
              ...
              <el-button
                plain
                size="large"
                :loading="assetBatchExportingScope === 'storyboard_videos'"
                :disabled="!currentEpisodeId || !!assetBatchExportingScope"
                @click="batchExportAssets('storyboard_videos')"
              >
                <el-icon v-if="assetBatchExportingScope !== 'storyboard_videos'"><Download /></el-icon>
                批量导出分镜视频
              </el-button>
        ...
        <template v-if="storyboards.length > 0">
          ...
          <!-- 分镜控制栏（卡片外，缩进表示属于当前幕） -->
          <div :id="'sb-' + sb.id" class="sb-ctrl-bar">
            ...
            <el-button
              size="small"
              plain
              class="sb-ctrl-btn sb-ctrl-config-btn"
              @click="onOpenVideoParamsDialog(sb)"
            >
              ⚙ 分镜配置
            </el-button>
            <el-button
              ...
              :title="isSbUniversalMode(sb.id) ? '切换为经典分镜（中间显示参考图）' : '切换为全能模式（中间显示片段描述）'"
              @click="onToggleSbUniversalMode(sb)"
            >
              {{ isSbUniversalMode(sb.id) ? '经典分镜' : '全能模式' }}
            </el-button>
            <el-checkbox
              :model-value="isSbFirstLastFrameMode(sb)"
              :disabled="isSbUniversalMode(sb.id)"
              class="sb-ctrl-first-last"
              size="small"
              @change="(v) => onToggleSbFirstLastFrame(sb, v)"
            >
              首尾帧参考图
            </el-checkbox>
          ...
          <div v-if="false" class="sb-workspace-editor">
            <div class="sb-workspace-editor__header">
              <div class="sb-workspace-editor__identity">
                <span class="sb-workspace-editor__shot-no">镜头 {{ i + 1 }}</span>
                <div>
                  <h3>分镜配置</h3>
                  <p>{{ sbTitle[sb.id] || sb.title || '未命名分镜' }}</p>
                </div>
              </div>
              <div class="sb-workspace-editor__header-tools">
                <label class="sb-editor-mode-control">
                  <span>本镜模式（优先全局）</span>
                  <el-radio-group
                    :model-value="sbCreationMode[sb.id] === 'universal' ? 'universal' : 'classic'"
                    size="small"
                    @change="(v) => setSbCreationModeId(sb.id, v)"
                  >
                    <el-radio-button value="classic">经典</el-radio-button>
                    <el-radio-button value="universal">全能分镜</el-radio-button>
                  </el-radio-group>
                </label>
                <el-checkbox
                  :model-value="isSbFirstLastFrameMode(sb)"
                  :disabled="isSbUniversalMode(sb.id)"
                  class="sb-editor-first-last-control"
                  @change="(v) => setSbUseFirstLastFrameId(sb.id, v)"
                >
                  首尾帧参考图
                </el-checkbox>
                <el-button
                  type="primary"
                  size="small"
                  :loading="savingStoryboardWorkspaceIds.has(sb.id)"
                  @click="onSaveStoryboardWorkspace(sb)"
                >
                  保存分镜配置
                </el-button>
              </div>
            ...
            <div class="sb-editor-section-heading">
              <div>
                <span class="sb-editor-card__step">01</span>
                <strong>基础参数</strong>
              </div>
              <span>设置场景信息与摄影参数</span>
            </div>
            <div class="sb-editor-basic-grid">
              <label class="sb-editor-field sb-editor-field--title">
                <span>镜头标题</span>
                <el-input v-model="sbTitle[sb.id]" placeholder="概括本镜头内容" />
              </label>
              <label class="sb-editor-field sb-editor-field--location">
                <span>地点</span>
                <el-input v-model="sbLocation[sb.id]" placeholder="场景地点" />
              </label>
              <label class="sb-editor-field">
                <span>时间</span>
                <el-input v-model="sbTime[sb.id]" placeholder="清晨 / 午后" />
              </label>
              <label class="sb-editor-field">
                <span>时长</span>
              ...
              <label class="sb-editor-field">
                <span>景别</span>
                <el-select v-model="sbShotType[sb.id]" placeholder="选择景别">
              ...
              <label class="sb-editor-field sb-editor-field--movement">
                <span>运镜</span>
                <el-select v-model="sbMovement[sb.id]" placeholder="推荐动态运镜" clearable filterable>
                  <el-option-group
                    v-for="group in STORYBOARD_MOVEMENT_OPTION_GROUPS"
                    :key="group.label"
                    :label="group.label"
                  >
                    <el-option
              ...
              <label class="sb-editor-field sb-editor-field--atmosphere">
                <span>氛围</span>
                <el-input v-model="sbAtmosphere[sb.id]" placeholder="情绪与氛围" />
              </label>
            </div>

            <details class="sb-editor-advanced">
              <summary>
                <span>高级摄影设置</span>
                <small>镜头视角、灯光、景深与空间布局</small>
              </summary>
              <div class="sb-editor-advanced__grid">
                <div class="sb-editor-field sb-editor-field--angle">
                  <span>镜头视角</span>
                  <div class="sb-editor-angle-row">
                    <el-select v-model="sbAngleS[sb.id]" placeholder="景别">
                      <el-option label="特写" value="close_up" />
                      <el-option label="中景" value="medium" />
                      <el-option label="远景" value="wide" />
                    </el-select>
                    <el-select v-model="sbAngleV[sb.id]" placeholder="俯仰">
                    ...
                    <el-select v-model="sbAngleH[sb.id]" placeholder="方向">
                      <el-option label="正面" value="front" />
                      <el-option label="前左 45°" value="front_left" />
                      <el-option label="左侧" value="left" />
                      <el-option label="后左 135°" value="back_left" />
                      <el-option label="背面" value="back" />
                      <el-option label="后右 135°" value="back_right" />
                      <el-option label="右侧" value="right" />
                      <el-option label="前右 45°" value="front_right" />
                    </el-select>
                  </div>
                </div>
                <label class="sb-editor-field">
                  <span>灯光</span>
                  <el-select v-model="sbLighting[sb.id]" placeholder="灯光风格" clearable>
                ...
                <label class="sb-editor-field">
                  <span>景深</span>
                  <el-select v-model="sbDof[sb.id]" placeholder="景深" clearable>
                ...
                <div class="sb-editor-field sb-editor-field--layout">
                  <span>空间布局锚点</span>
                  <div class="sb-editor-layout-row">
                    <el-input
                      v-model="sbLayoutDescription[sb.id]"
                      type="textarea"
                      :rows="2"
                      placeholder="描述人物站位与构图，用于锁定首尾帧的一致性"
                    />
                    <el-button
                      size="small"
                      :loading="regeneratingLayoutSbIds.has(sb.id)"
                      @click="onRegenerateLayoutDescription(sb)"
                    >
                      AI 优化
                    </el-button>
                  </div>
            ...
            <div class="sb-editor-content-grid">
              <section class="sb-editor-card sb-editor-card--script">
                <div class="sb-editor-card__title">
                  <div>
                    <span class="sb-editor-card__step">02</span>
                    <strong>镜头内容</strong>
                  </div>
                  <span>决定画面发生什么</span>
                </div>
                <div class="sb-editor-script-grid">
                  <label class="sb-editor-field">
                    <span>动作</span>
                    <el-input v-model="sbAction[sb.id]" type="textarea" :rows="3" placeholder="角色动作与画面变化" />
                  </label>
                  <label class="sb-editor-field">
                    <span>对白</span>
                    <el-input v-model="sbDialogue[sb.id]" type="textarea" :rows="3" placeholder="角色名：对白内容" />
                  </label>
                  <label class="sb-editor-field">
                    <span>解说旁白</span>
                    <el-input v-model="sbNarration[sb.id]" type="textarea" :rows="3" class="sb-narration-input" placeholder="画外音或纪录片式旁白" />
                  </label>
                  <label class="sb-editor-field">
                    <span>画面结果</span>
                    <el-input v-model="sbResult[sb.id]" type="textarea" :rows="3" placeholder="动作完成后的最终画面" />
                  </label>
                </div>
                <div v-if="canSplitSbByAudio(sb)" class="sb-editor-audio-tip">
                  <span>检测到多句对白或对白与旁白混用，可拆镜避免视频串音。</span>
                  <el-button size="small" type="warning" plain :loading="splitByAudioLoading" @click="onSplitSbByAudio(sb)">按对白拆镜</el-button>
                </div>
              </section>

              <section class="sb-editor-card sb-editor-card--prompts">
                <div class="sb-editor-card__title">
                  <div>
                    <span class="sb-editor-card__step">03</span>
                    <strong>提示词编辑</strong>
                  </div>
                  <span>图片原始、图片优化与视频提示词</span>
                </div>
                <div class="sb-editor-prompt-workspace">
                  <label class="sb-editor-field sb-editor-prompt-pane">
                    <span>原始提示词</span>
                    <el-input
                      v-model="sbImagePrompt[sb.id]"
                      type="textarea"
                      :rows="10"
                      class="sb-editor-prompt-textarea"
                      placeholder="描述主体、环境、构图与风格"
                    />
                  </label>
                  <div class="sb-editor-prompt-transform">
                    <el-button
                      type="warning"
                      plain
                      :loading="polishingStoryboardPromptIds.has(sb.id)"
                      @click="onPolishInlineSbPrompt(sb)"
                    >
                      <el-icon><MagicStick /></el-icon>
                      {{ sbPolishedPrompt[sb.id] ? '重新优化' : 'AI 优化' }}
                      <el-icon class="sb-editor-prompt-transform__arrow"><ArrowRight /></el-icon>
                    </el-button>
                  </div>
                  <label class="sb-editor-field sb-editor-prompt-pane">
                    <span>图片优化提示词</span>
                    <el-input
                      v-model="sbPolishedPrompt[sb.id]"
                      type="textarea"
                      :rows="10"
                      class="sb-editor-prompt-textarea"
                      placeholder="可手动编辑，或使用 AI 优化"
                    />
                  </label>
                  <label class="sb-editor-field sb-editor-prompt-pane sb-editor-prompt-pane--video">
                    <span>视频提示词</span>
                    <el-input
                      v-model="sbVideoPrompt[sb.id]"
                      type="textarea"
                      :rows="10"
                      class="sb-editor-prompt-textarea"
                      placeholder="控制动作、运镜与声音；留空可根据分镜配置重新生成"
                    />
                  </label>
                </div>
                <div class="sb-editor-video-prompt-footer">
                  <span>经典模式保存时会按最新分镜配置重建视频提示词；全能模式保存片段描述。</span>
                </div>
              </section>
            </div>
          </div>
          <div class="storyboard-row">
            ...
            <div class="sb-panel sb-script">
              <!-- 当前选中：场景 / 角色 / 物品缩略图 -->
              <div class="sb-selected-thumbs">
                <div class="sb-thumb-row">
                  <span class="sb-thumb-label">场景</span>
                  <div class="sb-thumb-list">
                    <div
                      v-if="getSbSelectedScene(sb.id)"
                      class="sb-thumb-item sb-thumb-scene"
                      :class="{ 'sb-thumb-clickable': hasAssetImage(getSbSelectedScene(sb.id)) }"
                      :title="getSbSelectedScene(sb.id).location"
                      role="button"
                      @click="hasAssetImage(getSbSelectedScene(sb.id)) && openImagePreview(assetImageUrl(getSbSelectedScene(sb.id)))"
                    >
                      <div class="sb-thumb-visual">
                        <img v-if="hasAssetImage(getSbSelectedScene(sb.id))" :src="assetImageUrl(getSbSelectedScene(sb.id))" alt="" />
                        <span v-else class="sb-thumb-placeholder">{{ (getSbSelectedScene(sb.id).location || '')[0] }}</span>
                      </div>
                      <span class="sb-thumb-item-name">{{ getSbSelectedScene(sb.id).location || '未命名场景' }}</span>
                      <button type="button" class="sb-thumb-remove" title="移除场景" @click.stop="onSbSelectSceneCommand(sb.id, '__clear__')">×</button>
                    </div>
                    <el-dropdown trigger="click" @command="(cmd) => onSbSelectSceneCommand(sb.id, cmd)">
                      <div class="sb-thumb-item sb-thumb-scene sb-thumb-add" title="选择场景" role="button" @click.stop>
                        <div class="sb-thumb-visual"><el-icon><Plus /></el-icon></div>
                        <span class="sb-thumb-item-name">选择场景</span>
                      </div>
                      <template #dropdown>
                        <el-dropdown-menu>
                          <el-dropdown-item v-for="s in (scenes || [])" :key="s.id" :command="s.id">
                            {{ s.location || '未命名场景' }}
                          </el-dropdown-item>
                          <el-dropdown-item v-if="!(scenes || []).length" disabled>暂无可选场景</el-dropdown-item>
                        </el-dropdown-menu>
                      </template>
                    </el-dropdown>
                  </div>
                ...
                <div class="sb-thumb-row">
                  ...
                  <div class="sb-thumb-list">
                    ...
                    >
                      ...
                      <span class="sb-thumb-item-name">{{ c.name || '未命名角色' }}</span>
                      <button type="button" class="sb-thumb-remove" title="移除角色" @click.stop="onSbRemoveCharacter(sb.id, c.id)">×</button>
                    </div>
                    <el-dropdown trigger="click" @command="(cmd) => onSbAddCharacterCommand(sb.id, cmd)">
                      <div
                        class="sb-thumb-item sb-thumb-avatar sb-thumb-add"
                        title="添加角色"
                        role="button"
                        @click.stop
                      >
                        ...
                        <span class="sb-thumb-item-name">添加角色</span>
                      </div>
                      <template #dropdown>
                ...
                <div class="sb-thumb-row">
                  ...
                  <div class="sb-thumb-list">
                    ...
                    >
                      ...
                      <span class="sb-thumb-item-name">{{ p.name || '未命名物品' }}</span>
                      <button type="button" class="sb-thumb-remove" title="移除物品" @click.stop="onSbRemoveProp(sb.id, p.id)">×</button>
                    </div>
                    <el-dropdown trigger="click" @command="(cmd) => onSbAddPropCommand(sb.id, cmd)">
                      <div class="sb-thumb-item sb-thumb-prop sb-thumb-add" title="添加物品" role="button" @click.stop>
                        <div class="sb-thumb-visual"><el-icon><Plus /></el-icon></div>
                        <span class="sb-thumb-item-name">添加物品</span>
                      </div>
                      <template #dropdown>
                        <el-dropdown-menu>
                          <el-dropdown-item
                            v-for="p in propsAvailableToAddToSb(sb.id)"
                            :key="p.id"
                            :command="p.id"
                          >
                            {{ p.name || '未命名物品' }}
                          </el-dropdown-item>
                          <el-dropdown-item v-if="!propsAvailableToAddToSb(sb.id).length" disabled>
                            已全部添加或无物品
                          </el-dropdown-item>
                        </el-dropdown-menu>
                      </template>
              ...
              <div v-if="!isSbFirstLastFrameMode(sb)" class="sb-prompt-row sb-image-prompt-inline">
                <el-input
                  v-model="sbImagePrompt[sb.id]"
                  type="textarea"
                  :rows="8"
                  class="sb-image-prompt-inline-input"
                  placeholder="输入图片提示词"
                />
                <el-button
                  size="small"
                  type="primary"
                  plain
                  :loading="savingSbImagePromptIds.has(sb.id)"
                  @click="onSaveSbImagePrompt(sb)"
                >
                  保存
                </el-button>
              </div>
            ...
            <div
              ...
              :class="{
                'sb-image--universal': isSbUniversalMode(sb.id),
                'sb-media-column--aligned': !isSbUniversalMode(sb.id) && !isSbFirstLastFrameMode(sb),
                'sb-media-column--first-last': !isSbUniversalMode(sb.id) && isSbFirstLastFrameMode(sb),
              }"
            >
              ...
              <div
                ...
                :class="{
                  ...
                  'sb-image-area--has-quad': !isSbFirstLastFrameMode(sb) && getStripItems(sb.id).length > 0,
                  'sb-image-area--first-last': isSbFirstLastFrameMode(sb),
                }"
              ...
              >
                <!-- 首尾帧双槽 -->
                <template v-if="isSbFirstLastFrameMode(sb)">
                  <div class="sb-fl-dual">
                    <div class="sb-fl-slot">
                      <div class="sb-fl-slot-label">首帧参考图</div>
                      <div class="sb-fl-slot-body">
                        <template v-if="getSbFirstImage(sb.id)">
                          <img
                            ...
                            alt=""
                            @click="openImagePreview(assetImageUrl(getSbFirstImage(sb.id)), getFramePreviewUrls(sb.id, 'first'))"
                          />
                        </template>
                        <template v-else-if="sb.image_url || sb.composed_image">
                          <img
                            ...
                            class="sb-generated-img"
                            alt=""
                            @click="openImagePreview(imageUrl(sb.composed_image || sb.image_url), getFramePreviewUrls(sb.id, 'first'))"
                          />
                        </template>
                        <template v-else>
                      ...
                      <div class="sb-fl-slot-prompt" :title="getSbFirstImage(sb.id)?.prompt || ''">
                        {{ getSbFirstImage(sb.id)?.prompt || '' }}
                      </div>
                      <div class="sb-fl-slot-actions">
                        <el-button type="primary" size="small" :loading="generatingSbFirstImageIds.has(sb.id)" @click="onGenerateSbFrameImage(sb, 'first')">
                          生成首帧
                        </el-button>
                      ...
                      <div class="sb-frame-history sb-frame-history--first">
                        <div class="sb-frame-history__header">
                          <div>
                            <strong>首帧历史</strong>
                            <span>{{ getFrameHistoryItems(sb.id, 'first').length }} 张</span>
                          </div>
                          <div v-if="getFrameHistoryItems(sb.id, 'first').length > 1" class="sb-frame-history__nav">
                            <button type="button" title="首帧历史上一组" @click="scrollFrameHistory(sb.id, 'first', -1)">
                              <el-icon><ArrowLeft /></el-icon>
                            </button>
                            <button type="button" title="首帧历史下一组" @click="scrollFrameHistory(sb.id, 'first', 1)">
                              <el-icon><ArrowRight /></el-icon>
                            </button>
                          </div>
                        </div>
                        <div
                          v-if="getFrameHistoryItems(sb.id, 'first').length"
                          :id="`sb-frame-history-${sb.id}-first`"
                          class="sb-frame-history__track"
                        >
                          <div
                            ...
                            role="button"
                            tabindex="0"
                            @click="onSelectFrameHistoryItem(sb, item, 'first')"
                            @keydown.enter.self="onSelectFrameHistoryItem(sb, item, 'first')"
                          >
                            <div class="sb-frame-history-card__preview">
                              <img :src="item.src" alt="首帧历史参考图" />
                            </div>
                            <span class="sb-frame-history-card__label">{{ item.label }}</span>
                            <button type="button" class="thumb-preview-btn" title="放大预览" @click.stop="openImagePreview(item.src, getFramePreviewUrls(sb.id, 'first'))">
                              <el-icon :size="10"><ZoomIn /></el-icon>
                            </button>
                            <button v-if="item.img?.id && !item.isCurrent" type="button" class="extra-thumb-remove" title="删除首帧历史图" @click.stop="onRemoveSbHistoryImage(sb.id, item.img.id)">×</button>
                          </div>
                        </div>
                        <div v-else class="sb-frame-history__empty">暂无首帧历史图</div>
                      </div>
                    </div>
                    <div class="sb-fl-arrow" aria-hidden="true">→</div>
                    <div class="sb-fl-slot">
                      <div class="sb-fl-slot-label">尾帧参考图</div>
                      <div class="sb-fl-slot-body sb-fl-slot-body--last">
                        <el-checkbox
                          v-model="lastFrameUseFirstLayoutLock"
                          class="sb-fl-first-lock-opt sb-fl-first-lock-opt--overlay"
                          title="勾选时尾帧生成会附带首帧图作构图与左右站位参考；取消后仅使用场景/角色/道具参考，便于调整出场人物"
                          @change="onLastFrameLayoutLockChange"
                        >
                          <span class="sb-fl-first-lock-text">首帧<br />站位</span>
                        </el-checkbox>
                        <template v-if="getSbLastImage(sb.id)">
                          <img
                            ...
                            @click="openImagePreview(assetImageUrl(getSbLastImage(sb.id)), getFramePreviewUrls(sb.id, 'last'))"
                          />
                        </template>
                        <template v-else>
                      ...
                      <div class="sb-fl-slot-prompt" :title="getSbLastImage(sb.id)?.prompt || ''">
                        {{ getSbLastImage(sb.id)?.prompt || '' }}
                      </div>
                      <div class="sb-fl-slot-actions">
                        <el-button type="primary" size="small" :loading="generatingSbLastImageIds.has(sb.id)" @click="onGenerateSbFrameImage(sb, 'last')">
                          {{ getSbLastImage(sb.id) ? '重新生成尾帧' : '生成尾帧' }}
                        </el-button>
                      ...
                      <div class="sb-frame-history sb-frame-history--last">
                        <div class="sb-frame-history__header">
                          <div>
                            <strong>尾帧历史</strong>
                            <span>{{ getFrameHistoryItems(sb.id, 'last').length }} 张</span>
                          </div>
                          <div v-if="getFrameHistoryItems(sb.id, 'last').length > 1" class="sb-frame-history__nav">
                            <button type="button" title="尾帧历史上一组" @click="scrollFrameHistory(sb.id, 'last', -1)">
                              <el-icon><ArrowLeft /></el-icon>
                            </button>
                            <button type="button" title="尾帧历史下一组" @click="scrollFrameHistory(sb.id, 'last', 1)">
                              <el-icon><ArrowRight /></el-icon>
                            </button>
                          </div>
                        </div>
                        <div
                          v-if="getFrameHistoryItems(sb.id, 'last').length"
                          :id="`sb-frame-history-${sb.id}-last`"
                          class="sb-frame-history__track"
                        >
                          <div
                            ...
                            role="button"
                            tabindex="0"
                            @click="onSelectFrameHistoryItem(sb, item, 'last')"
                            @keydown.enter.self="onSelectFrameHistoryItem(sb, item, 'last')"
                          >
                            <div class="sb-frame-history-card__preview">
                              <img :src="item.src" alt="尾帧历史参考图" />
                            </div>
                            <span class="sb-frame-history-card__label">{{ item.label }}</span>
                            <button type="button" class="thumb-preview-btn" title="放大预览" @click.stop="openImagePreview(item.src, getFramePreviewUrls(sb.id, 'last'))">
                              <el-icon :size="10"><ZoomIn /></el-icon>
                            </button>
                            <button v-if="item.img?.id && !item.isCurrent" type="button" class="extra-thumb-remove" title="删除尾帧历史图" @click.stop="onRemoveSbHistoryImage(sb.id, item.img.id)">×</button>
                          </div>
                        </div>
                        <div v-else class="sb-frame-history__empty">暂无尾帧历史图</div>
                      </div>
                    </div>
                  </div>
                </template>
                ...
                <div class="sb-main-image-wrap">
                  ...
                  <template v-else-if="getSbImageError(sb.id) || sb.error_msg || sb.errorMsg">
                    <div class="sb-image-error" :title="getSbImageError(sb.id) || sb.error_msg || sb.errorMsg">
                      {{ getSbImageError(sb.id) || sb.error_msg || sb.errorMsg }}
                    </div>
                  </template>
                  <template v-else>
                    <span class="sb-media-empty-text">暂无分镜图片</span>
                  </template>
                </div>
                </template>
                <div v-if="dragOverSbId === sb.id" class="sb-image-area-drop-hint">松开上传到首帧</div>
              </div>
              <div v-if="!isSbFirstLastFrameMode(sb)" class="sb-image-history sb-media-history">
                <div class="sb-image-history__header">
                  <strong>历史分镜图片</strong>
                  <span>{{ getStripItems(sb.id).length }} 张</span>
                </div>
                <div v-if="getStripItems(sb.id).length" class="sb-imgs-strip">
                  <el-tooltip content="全部分镜图：点击切换主图，当前图会始终保留在这里" placement="top" :show-arrow="false">
                  ...
                  >
                    ...
                    <button v-if="item.img?.id && !item.isCurrent" class="extra-thumb-remove" title="删除历史图" @click.stop="onRemoveSbHistoryImage(sb.id, item.img.id)">×</button>
                  </div>
                </div>
                <div v-else class="sb-image-history__empty">暂无历史分镜图片</div>
              </div>
              <div class="sb-image-actions sb-media-actions">
                <template v-if="isSbFirstLastFrameMode(sb)">
                  <el-button size="small" :loading="generatingSbFirstImageIds.has(sb.id) || generatingSbLastImageIds.has(sb.id)" @click="onGenerateSbFramePair(sb)">
                    {{ hasSbImageGenerationHistory(sb.id) || hasSbImage(sb) ? '重新生成分镜图（首尾帧）' : '生成分镜图（首尾帧）' }}
                  </el-button>
                ...
                <template v-else>
                <el-button size="small" :loading="generatingSbImageIds.has(sb.id)" @click="onGenerateSbImage(sb)">
                  {{ hasSbImage(sb) ? '重新生成分镜图' : '生成分镜图' }}
                </el-button>
              ...
              <div v-if="!isSbFirstLastFrameMode(sb)" class="sb-polished-prompt-section sb-media-prompt-section">
                <div class="sb-polished-prompt-label">
                  <div>
                    <span class="sb-dot"></span>
                    <span>图片优化提示词</span>
                  </div>
                ...
                <div class="sb-polished-prompt-bar">
                  <el-input
                    v-model="sbPolishedPrompt[sb.id]"
                    type="textarea"
                    :rows="8"
                    class="sb-polished-prompt-input"
                    placeholder="输入图片优化提示词，或使用 AI 优化"
                  />
                  <el-button
                    size="small"
                    type="primary"
                    plain
                    :loading="savingSbPolishedPromptIds.has(sb.id)"
                    @click="onSaveSbPolishedPrompt(sb)"
                  >
                    保存
                  </el-button>
                </div>
            ...
            <div class="sb-panel sb-video sb-media-column--fixed sb-media-column--aligned">
              ...
              <div v-else class="sb-video-area sb-video-placeholder">
                ...
                <template v-else>
                  ...
                  <span v-else class="sb-media-empty-text">暂无分镜视频</span>
                </template>
              </div>
              <div class="sb-video-history sb-media-history">
                <div class="sb-video-history__header">
                  <strong>历史分镜视频</strong>
                  <span>{{ getVideoStripItems(sb.id).length }} 个</span>
                </div>
                <div v-if="getVideoStripItems(sb.id).length" class="sb-videos-strip">
                  <el-tooltip content="全部分镜视频：点击切换，当前视频会始终保留在这里" placement="top" :show-arrow="false">
                    <el-icon class="sb-strip-hint-icon"><InfoFilled /></el-icon>
                  </el-tooltip>
                    <div
                      ...
                      @keydown.enter="onSelectSbMainVideo(sb, item.video)"
                    >
                      <video :src="item.src" preload="metadata" class="sb-video-thumb-player" />
                      <span class="sb-video-thumb-label">{{ item.label }}</span>
                    </div>
                </div>
                <div v-else class="sb-video-history__empty">暂无历史分镜视频</div>
              </div>
              <div class="sb-video-actions sb-media-actions">
                <el-button size="small" :loading="isSbVideoGenerating(sb.id)" :disabled="!sbCanSubmitVideo(sb) || isSbVideoGenerating(sb.id)" @click="onGenerateSbVideo(sb)">
                  {{ getSbVideo(sb.id) || hasSbVideoGenerationHistory(sb.id) ? '重新生成' : '生成分镜视频' }}
                </el-button>
                <el-tooltip v-if="getNextStoryboard(sb.id)" content="提取本视频尾帧，设为下一个分镜的首帧" placement="top">
                  <el-button size="small" :disabled="!getSbVideo(sb.id)" :loading="linkingTailFrameIds.has(sb.id)" @click="onLinkTailFrameToNext(sb)">尾帧衔接</el-button>
                </el-tooltip>
              ...
              <div class="sb-video-prompt-section sb-media-prompt-section">
                ...
                <div class="sb-video-params-bar">
                  <el-input
                    ...
                    type="textarea"
                    :rows="8"
                    class="sb-video-prompt-inline-input"
                    placeholder="输入视频提示词"
                  />
                  <el-button
                    size="small"
                    type="primary"
                    plain
                    :loading="savingSbVideoPromptIds.has(sb.id)"
                    @click="onSaveSbVideoPrompt(sb)"
                  >
                    保存
                  </el-button>
                </div>
    ...
    <!-- 角色模板与剧集角色 -->
    <el-dialog v-model="showCharLibrary" title="角色模板与复用" width="720px" destroy-on-close class="library-dialog" @open="onCharLibraryDialogOpen">
      <el-tabs v-model="charLibraryTab" class="char-library-tabs" @tab-change="onCharLibraryTabChange">
        <el-tab-pane label="可复用角色模板" name="library">
          <div class="library-scope-tip"><strong>主动保存的本剧标准角色。</strong>用于后续各集保持角色形象一致，不会自动包含所有已创建角色。</div>
          ...
          <div v-loading="charLibraryLoading" class="library-list">
            ...
            <div v-if="!charLibraryLoading && charLibraryList.length === 0" class="library-empty">暂无可复用角色模板。请先为剧集角色生成图片，再点击「保存为可复用模板」</div>
          </div>
        ...
        <el-tab-pane label="各集中已创建的角色" name="drama">
          <div class="library-scope-tip"><strong>系统自动汇总的剧集制作记录。</strong>包含本剧各集从剧本提取或手动添加的角色，无需先保存为模板。</div>
          <div class="library-toolbar">
          ...
          <div v-loading="dramaAllCharLoading" class="library-list">
            ...
            <div v-if="!dramaAllCharLoading && dramaAllCharList.length === 0" class="library-empty">各集尚未创建角色，请先从剧本提取或手动添加角色</div>
          </div>
          <div class="library-pagination">
            <el-pagination
    ...
    <!-- 编辑可复用角色模板 -->
    <el-dialog v-model="showEditCharLibrary" title="编辑可复用角色模板" width="440px" @close="editCharLibraryForm = null">
    ...
    <!-- 道具模板与剧集道具 -->
    <el-dialog v-model="showPropLibrary" title="道具模板与复用" width="720px" destroy-on-close class="library-dialog" @open="onPropLibraryDialogOpen">
      <el-tabs v-model="propLibraryTab" class="char-library-tabs" @tab-change="onPropLibraryTabChange">
        <el-tab-pane label="可复用道具模板" name="library">
          <div class="library-scope-tip"><strong>主动保存的本剧标准道具。</strong>用于后续各集保持道具外观一致，不会自动包含所有已创建道具。</div>
          <div class="library-toolbar">
          ...
          <div v-loading="propLibraryLoading" class="library-list">
            ...
            <div v-if="!propLibraryLoading && propLibraryList.length === 0" class="library-empty">暂无可复用道具模板。请先为剧集道具生成图片，再点击「保存为可复用模板」</div>
          </div>
          <div class="library-pagination">
        ...
        <el-tab-pane label="各集中已创建的道具" name="drama">
          <div class="library-scope-tip"><strong>系统自动汇总的剧集制作记录。</strong>包含本剧各集从剧本提取或手动添加的道具，无需先保存为模板。</div>
          <div class="library-toolbar">
          ...
          <div v-loading="dramaAllPropLoading" class="library-list">
            ...
            <div v-if="!dramaAllPropLoading && dramaAllPropList.length === 0" class="library-empty">各集尚未创建道具，请先从剧本提取或手动添加道具</div>
          </div>
          <div class="library-pagination">
    ...
    <!-- 编辑可复用道具模板 -->
    <el-dialog v-model="showEditPropLibrary" title="编辑可复用道具模板" width="440px" @close="editPropLibraryForm = null">
    ...
    <!-- 场景模板与剧集场景 -->
    <el-dialog v-model="showSceneLibrary" title="场景模板与复用" width="720px" destroy-on-close class="library-dialog" @open="onSceneLibraryDialogOpen">
      <el-tabs v-model="sceneLibraryTab" class="char-library-tabs" @tab-change="onSceneLibraryTabChange">
        <el-tab-pane label="可复用场景模板" name="library">
          <div class="library-scope-tip"><strong>主动保存的本剧标准场景。</strong>用于后续各集保持场景环境一致，不会自动包含所有已创建场景。</div>
          <div class="library-toolbar">
          ...
          <div v-loading="sceneLibraryLoading" class="library-list">
            ...
            <div v-if="!sceneLibraryLoading && sceneLibraryList.length === 0" class="library-empty">暂无可复用场景模板。请先为剧集场景生成图片，再点击「保存为可复用模板」</div>
          </div>
          <div class="library-pagination">
        ...
        <el-tab-pane label="各集中已创建的场景" name="drama">
          <div class="library-scope-tip"><strong>系统自动汇总的剧集制作记录。</strong>包含本剧各集从剧本提取或手动添加的场景，无需先保存为模板。</div>
          <div class="library-toolbar">
          ...
          <div v-loading="dramaAllSceneLoading" class="library-list">
            ...
            <div v-if="!dramaAllSceneLoading && dramaAllSceneList.length === 0" class="library-empty">各集尚未创建场景，请先从剧本提取或手动添加场景</div>
          </div>
          <div class="library-pagination">
    ...
    <!-- 编辑可复用场景模板 -->
    <el-dialog v-model="showEditSceneLibrary" title="编辑可复用场景模板" width="440px" @close="editSceneLibraryForm = null">
    ...
    <!-- 分镜配置：保持主卡片紧凑，逐镜配置优先于全局默认 -->
    <el-dialog
      v-model="showVideoParamsDialog"
      :title="`分镜 ${videoParamsTarget?.storyboard_number ?? ''} · 分镜配置`"
      width="min(1180px, 96vw)"
      top="5vh"
      append-to-body
      destroy-on-close
      class="storyboard-config-dialog"
      body-class="storyboard-config-dialog__body"
      @opened="restoreVideoParamsPageScroll"
      @closed="onVideoParamsDialogClosed"
    >
      <template v-if="videoParamsTarget">
        <div class="vp-dialog-layout">
          <section class="vp-params-column">
            <el-form label-width="88px" size="small" class="vp-dialog-form">
              <el-divider content-position="left">基础参数</el-divider>
              <el-row :gutter="12">
                <el-col :span="12">
                  <el-form-item label="标题">
                    <el-input
                      v-model="sbTitle[videoParamsTarget.id]"
                      type="textarea"
                      :rows="2"
                      resize="none"
                      placeholder="镜头标题"
                ...
                <el-col :span="12">
                  <el-form-item label="地点">
                    <el-input
                      v-model="sbLocation[videoParamsTarget.id]"
                      type="textarea"
                      :rows="2"
                      resize="none"
                      placeholder="场景地点"
              ...
              <el-row :gutter="12">
                <el-col :span="12">
                  <el-form-item label="时间">
                    <el-input
                      v-model="sbTime[videoParamsTarget.id]"
                      type="textarea"
                      :rows="2"
                      resize="none"
                      placeholder="清晨 / 午后"
                ...
                <el-col :span="12">
                  <el-form-item label="氛围">
                    <el-input
                      v-model="sbAtmosphere[videoParamsTarget.id]"
                      type="textarea"
                      :rows="2"
                      resize="none"
                      placeholder="氛围 / 情绪"
              ...
              <el-row :gutter="12">
                <el-col :span="12">
                  <el-form-item label="时长(秒)">
                    <el-input-number
                      v-model="sbDuration[videoParamsTarget.id]"
                      :min="4"
                      :max="15"
                      :step="1"
                      controls-position="right"
          ...
          <el-row :gutter="12">
            <el-col :span="12">
              <el-form-item label="运镜">
                <el-select v-model="sbMovement[videoParamsTarget.id]" placeholder="推荐动态运镜" style="width:100%" clearable filterable>
                  <el-option-group
                    v-for="group in STORYBOARD_MOVEMENT_OPTION_GROUPS"
                    :key="group.label"
                    :label="group.label"
            ...
            <el-col :span="12">
              <el-form-item label="镜头视角">
                <div class="vp-angle-selects">
                  <el-select v-model="sbAngleS[videoParamsTarget.id]" placeholder="景别">
                    <el-option label="特写" value="close_up" />
                    <el-option label="中景" value="medium" />
                    <el-option label="远景" value="wide" />
                  </el-select>
                  <el-select v-model="sbAngleV[videoParamsTarget.id]" placeholder="俯仰">
                    <el-option label="平视" value="eye_level" />
                    <el-option label="低角仰拍" value="low" />
                    <el-option label="高角俯拍" value="high" />
                    <el-option label="虫眼仰视" value="worm" />
                  </el-select>
                  <el-select v-model="sbAngleH[videoParamsTarget.id]" placeholder="方向">
                    <el-option label="正面" value="front" />
                    <el-option label="前左 45°" value="front_left" />
                    <el-option label="左侧" value="left" />
                    <el-option label="后左 135°" value="back_left" />
          ...
          <el-form-item label="空间布局锚点">
            <div class="vp-layout-row">
              <el-input
                v-model="sbLayoutDescription[videoParamsTarget.id]"
                type="textarea"
                :rows="2"
                placeholder="人物站位、距离与构图；用于锁定首尾帧一致性"
              />
              <el-button
          ...
          <div class="vp-content-section">
            <el-divider content-position="left">镜头内容</el-divider>
            <div class="vp-content-grid">
              <el-form-item label="动作">
                <el-input
                  v-model="sbAction[videoParamsTarget.id]"
                  type="textarea"
                  :rows="3"
                  resize="none"
                  placeholder="角色动作与画面变化"
                />
              </el-form-item>
              <el-form-item label="对白">
                <el-input
                  v-model="sbDialogue[videoParamsTarget.id]"
                  type="textarea"
                  :rows="3"
                  resize="none"
              ...
              <el-form-item label="解说旁白">
                <el-input
                  v-model="sbNarration[videoParamsTarget.id]"
                  type="textarea"
                  :rows="3"
                  resize="none"
                  placeholder="画外音或纪录片式旁白"
                />
              </el-form-item>
              <el-form-item label="画面结果">
                <el-input
                  v-model="sbResult[videoParamsTarget.id]"
                  type="textarea"
                  :rows="3"
                  resize="none"
                  placeholder="动作完成后的最终画面"
                />
              </el-form-item>
            </div>
            <el-form-item
              v-if="canSplitSbByAudio(videoParamsTarget)"
              label="对白拆镜"
              class="vp-split-audio-item"
            >
              <div class="sb-split-audio-row">
                <p class="sb-split-audio-tip">检测到多句对白或“对白 + 旁白”，可拆镜避免同镜串音。</p>
                <el-button type="warning" plain :loading="splitByAudioLoading" @click="onSplitSbByAudio(videoParamsTarget)">
                  按对白拆镜
                </el-button>
              </div>
            </el-form-item>
          ...
          <aside class="vp-video-prompt-column">
            <div class="vp-video-prompt-heading">
              <strong>视频提示词</strong>
              <span>当前提示词</span>
            </div>
            <div class="vp-video-prompt-preview">
              <div class="vp-video-prompt-hint">
                {{ isSbUniversalMode(videoParamsTarget.id)
                  ? '全能模式以卡片中的“片段描述”为主提示词。'
                  : '保存后会根据以上字段按最新规则自动更新。' }}
              </div>
              <el-input
                :model-value="sbVideoPrompt[videoParamsTarget.id] || videoParamsTarget.video_prompt || ''"
                type="textarea"
                class="vp-video-prompt-textarea"
                readonly
              />
            </div>
          </aside>
      ...
      <template #footer>
        ...
        <el-button
          type="primary"
          :loading="videoParamsSaving || savingStoryboardWorkspaceIds.has(videoParamsTarget?.id)"
          @click="onSaveVideoParams"
        >
          保存分镜配置
        </el-button>
      </template>
    ...
    <Teleport to="body">
      <div
        ...
        @keydown="onImagePreviewKeydown"
      >
        <button class="image-preview-close" type="button" title="关闭预览" @click.stop="closeImagePreview">
        ...
        <div class="image-preview-stage" @click.stop>
          ...
          <span v-if="previewImageUrls.length > 1" class="image-preview-counter">
...
} = useScenes({ store, dramaId, currentEpisodeId, getSelectedStyle, loadDrama, pollTask, pollUntilResourceHasImage, hasAssetImage, dramaAPI })
...
const assetBatchGenerationScope = ref('')
const assetBatchExportingScope = ref('')
const assetBatchProgress = ref({ current: 0, total: 0, failed: 0 })

const ASSET_BATCH_SCOPE_META = Object.freeze({
  all: {
    kinds: new Set(['character', 'scene', 'prop']),
    label: '资产',
    emptyLabel: '当前集所有资产',
  },
  characters: {
    kinds: new Set(['character']),
    label: '角色',
    emptyLabel: '当前集所有角色',
  },
  scenes: {
...
const ASSET_EXPORT_SCOPE_META = Object.freeze({
  all: { suffix: '资产', success: '全部资产已按名称打包导出' },
  characters: { suffix: '角色资产', success: '角色资产已批量导出' },
  scenes: { suffix: '场景资产', success: '场景资产已批量导出' },
  props: { suffix: '道具资产', success: '道具资产已批量导出' },
  storyboard_videos: { suffix: '分镜视频', success: '分镜视频已批量导出为 MP4' },
})

function currentAssetById(kind, id) {
...
async function generateBatchAsset(job) {
  const loadingSet = job.kind === 'character'
    ? generatingCharIds
    : job.kind === 'scene'
      ? generatingSceneIds
      : generatingPropIds
  const resourceType = job.kind === 'character'
    ? GEN_RESOURCE.CHAR_IMAGE
    : job.kind === 'scene'
      ? GEN_RESOURCE.SCENE_IMAGE
      : GEN_RESOURCE.PROP_IMAGE
  loadingSet.add(job.item.id)
  try {
    let res
    if (job.kind === 'character') {
      res = await characterAPI.generateImage(job.item.id, undefined, getSelectedStyle())
    } else if (job.kind === 'scene') {
      res = await sceneAPI.generateImage({
        scene_id: job.item.id,
        model: undefined,
        style: getSelectedStyle(),
        use_quad_grid: !!sceneUseQuadGrid.value,
      })
    } else {
      res = await propAPI.generateImage(
        job.item.id,
        undefined,
        getSelectedStyle(),
    ...
    if (taskId) {
      const result = await pollTask(taskId, () => loadDrama(), {
        resourceType,
        resourceId: job.item.id,
        label: `批量生成${job.label}`,
      })
      if (result?.status !== 'completed') {
        throw new Error(result?.error || `${job.label}生成失败`)
      }
    } else {
      await loadDrama()
      await pollUntilResourceHasImage(() => hasAssetImage(currentAssetById(job.kind, job.item.id)))
      if (!hasAssetImage(currentAssetById(job.kind, job.item.id))) {
        throw new Error(`${job.label}生成超时`)
      }
    }
  } finally {
    loadingSet.delete(job.item.id)
  }
}

async function startBatchAssetGeneration(scope = 'all') {
  if (!currentEpisodeId.value || assetBatchGenerating.value || pipelineRunning.value) return
  const scopeMeta = ASSET_BATCH_SCOPE_META[scope] || ASSET_BATCH_SCOPE_META.all
  const allJobs = [
    ...(characters.value || []).map((item) => ({
      kind: 'character',
      item,
      label: `角色「${item.name || item.id}」`,
    })),
    ...(scenes.value || []).map((item) => ({
      kind: 'scene',
      item,
      label: `场景「${item.location || item.id}」`,
    })),
    ...(props.value || []).map((item) => ({
      kind: 'prop',
      item,
      label: `道具「${item.name || item.id}」`,
    })),
  ]
  const selectedJobs = allJobs.filter((job) => scopeMeta.kinds.has(job.kind))
  const jobs = selectedJobs.filter((job) => !hasAssetImage(job.item))

  if (jobs.length === 0) {
    ElMessage.info(
      selectedJobs.length > 0
        ? `${scopeMeta.emptyLabel}均已有图片，无需生成`
        : `当前集暂无可生成的${scopeMeta.label}`
    )
    return
  }

  assetBatchGenerating.value = true
  assetBatchGenerationScope.value = scope
  assetBatchProgress.value = { current: 0, total: jobs.length, failed: 0 }
  const errors = []
  try {
    await runConcurrently(
      jobs,
      pipelineConcurrency.value || 3,
      async (job) => {
        try {
          await generateBatchAsset(job)
        } catch (e) {
          errors.push(`${job.label}：${e.message || '生成失败'}`)
          assetBatchProgress.value.failed += 1
        } finally {
          assetBatchProgress.value.current += 1
        }
      },
      { getLabel: (job) => job.label }
    )
    await loadDrama()
    if (errors.length > 0) {
      ElMessage.warning(
        `批量生成完成：成功 ${jobs.length - errors.length} 个，失败 ${errors.length} 个。${errors[0]}`
      )
    } else {
      ElMessage.success(`${scopeMeta.label}批量生成完成，共生成 ${jobs.length} 个`)
    }
  } finally {
    assetBatchGenerating.value = false
    assetBatchGenerationScope.value = ''
  }
}
...
async function batchExportAssets(scope = 'all') {
  if (!dramaId.value || !currentEpisodeId.value || assetBatchExportingScope.value) return
  const scopeMeta = ASSET_EXPORT_SCOPE_META[scope] || ASSET_EXPORT_SCOPE_META.all
  assetBatchExportingScope.value = scope
  try {
    const blob = await dramaAPI.exportAssets(dramaId.value, currentEpisodeId.value, scope)
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    const dramaName = safeDownloadName(store.drama?.title, '短剧')
    const episodeName = safeDownloadName(
      store.currentEpisode?.title || `第${store.currentEpisode?.episode_number || currentEpisodeId.value}集`,
      '当前集'
    )
    link.href = url
    link.download = `${dramaName}-${episodeName}-${scopeMeta.suffix}.zip`
    document.body.appendChild(link)
    link.click()
    link.remove()
    setTimeout(() => URL.revokeObjectURL(url), 1000)
    ElMessage.success(scopeMeta.success)
  } catch (e) {
    if (!e.response) ElMessage.error(e.message || '批量导出失败')
  } finally {
    assetBatchExportingScope.value = ''
  }
}
...
/** 经典首尾帧逐镜开关（存库 storyboards.use_first_last_frame；旧数据回退项目全局设置） */
const sbUseFirstLastFrame = ref({})
/** 全能模式片段描述（存库 universal_segment_text，与经典参考图字段独立） */
const sbUniversalSegmentText = ref({})
/** 同页编辑工作区中的图片、优化图片与视频提示词草稿 */
const sbImagePrompt = ref({})
const sbPolishedPrompt = ref({})
const sbVideoPrompt = ref({})
const savingStoryboardWorkspaceIds = reactive(new Set())
const polishingStoryboardPromptIds = reactive(new Set())
const showVideoParamsDialog = ref(false)
const videoParamsTarget = ref(null)
const videoParamsSaving = ref(false)
let videoParamsPageScrollPosition = null
...
const savingSbImagePromptIds = reactive(new Set())
const savingSbVideoPromptIds = reactive(new Set())
const savingSbPolishedPromptIds = reactive(new Set())
/** 首尾帧提示词编辑器 */
...
function openImagePreview(url, group = []) {
  const urls = [...new Set([...(Array.isArray(group) ? group : []), url].filter(Boolean))]
  const index = Math.max(0, urls.indexOf(url))
  previewImageUrls.value = urls
  previewImageIndex.value = index
  previewImageUrl.value = urls[index] || url
  nextTick(() => previewOverlayRef.value?.focus())
}
function stepImagePreview(offset) {
  const total = previewImageUrls.value.length
  if (total < 2) return
  const index = (previewImageIndex.value + offset + total) % total
  previewImageIndex.value = index
  previewImageUrl.value = previewImageUrls.value[index]
}
function onImagePreviewKeydown(event) {
...
function hasSbImageGenerationHistory(storyboardId) {
  const list = sbImages.value[storyboardId]
  return Array.isArray(list) && list.length > 0
}
function getSbImageError(storyboardId) {
  const list = sbImages.value[storyboardId]
  if (!Array.isArray(list) || list.length === 0) return ''
  const failed = list.find((i) => i.status === 'failed' && i.error_msg)
  return failed?.error_msg || ''
}
/** 取当前主图（首尾帧模式下等同首帧） */
function getSbImage(storyboardId) {
  if (isSbFirstLastFrameMode(storyboardId)) return getSbFirstImage(storyboardId)
  const images = getSbAllImages(storyboardId)
...
function hasSbVideoGenerationHistory(storyboardId) {
  const list = sbVideos.value[storyboardId]
  return Array.isArray(list) && list.length > 0
}
...
function isSameMediaRecord(left, right) {
  if (!left || !right) return false
  if (left.id != null && right.id != null) {
    return String(left.id) === String(right.id)
  }
  return ['local_path', 'image_url', 'video_url'].some((field) => {
...
function getVideoStripItems(storyboardId) {
  ...
  return all
    .map((v, idx) => {
      const isCurrent = isSameMediaRecord(v, current)
      return {
        key: `vid-${v.id ?? idx}`,
        video: v,
        src: assetVideoUrl(v),
...
async function loadStoryboardMedia() {
  ...
  const media = await episodeMediaAPI.get(episodeId, boards.map((sb) => sb.id))
  if (Number(currentEpisodeId.value) !== Number(episodeId)) return
  ...
  for (const sb of boards) {
    nextImages[sb.id] = imageGroups[String(sb.id)] || []
    nextVideos[sb.id] = videoGroups[String(sb.id)] || []
  }
...
/** 获取全部已完成图片；当前主图也保留在历史条中，便于切换后找回 */
function getStripItems(storyboardId) {
  const allImgs = getSbAllImages(storyboardId)
  const useFirstLast = isSbFirstLastFrameMode(storyboardId)
  const firstImg = useFirstLast ? getSbFirstImage(storyboardId) : getSbImage(storyboardId)
  const lastImg = useFirstLast ? getSbLastImage(storyboardId) : null
  let historyIndex = 0
  return allImgs
    .map((img, idx) => {
      const isCurrentFirst = isSameMediaRecord(img, firstImg)
      const isCurrentLast = isSameMediaRecord(img, lastImg)
      const isCurrent = isCurrentFirst || isCurrentLast
      const currentLabel = isCurrentFirst
        ? (useFirstLast ? '当前首帧' : '当前')
        : isCurrentLast
          ? '当前尾帧'
          : null
      return {
...
/** 首尾帧模式下按生成类型拆分历史图片；旧版未标类型的图片归入首帧历史 */
function getFrameHistoryItems(storyboardId, slot) {
  const items = getStripItems(storyboardId)
  const groups = splitStoryboardFrameHistory(items.map((item) => item.img))
  const itemById = new Map(items.map((item) => [Number(item.img?.id), item]))
  let historyIndex = 0
  return groups[slot === 'last' ? 'last' : 'first']
    .map((image) => itemById.get(Number(image.id)))
    .filter(Boolean)
    .map((item) => ({
      ...item,
...
function getFramePreviewUrls(storyboardId, slot) {
  const sb = (store.storyboards || []).find((item) => Number(item.id) === Number(storyboardId))
  const current = slot === 'last' ? getSbLastImage(storyboardId) : getSbFirstImage(storyboardId)
  const currentUrl = current
    ? assetImageUrl(current)
    : slot === 'first' && (sb?.composed_image || sb?.image_url)
      ? imageUrl(sb.composed_image || sb.image_url)
      : ''
  return [...new Set([
    currentUrl,
    ...getFrameHistoryItems(storyboardId, slot).map((item) => item.src),
  ].filter(Boolean))]
}

function onSelectFrameHistoryItem(sb, item, slot) {
  onSelectSbFrameImage(sb, item.img, slot)
  ElMessage.success(slot === 'last' ? '已设为尾帧参考图' : '已设为首帧参考图')
}

function scrollFrameHistory(storyboardId, slot, direction) {
  const track = document.getElementById(`sb-frame-history-${storyboardId}-${slot}`)
  if (!track) return
  track.scrollBy({ left: direction * Math.max(220, track.clientWidth * 0.8), behavior: 'smooth' })
}
...
function onSelectSbFrameImage(sb, img, slot) {
  ...
  // 这样点击后立即生效，无需刷新页面；历史条会保留全部图片并更新“当前”标识
...
function missingSbFrameSlots(sb) {
  if (!isSbFirstLastFrameMode(sb)) return []
  const slots = []
  const hasFirst = !!(getSbFirstImage(sb.id) || sb.image_url || sb.composed_image)
  if (!hasFirst) slots.push('first')
  if (!getSbLastImage(sb.id)) slots.push('last')
  return slots
}

/** 批量/流水线首尾帧生成：每个槽位先生成专业帧提示词，再提交对应图片任务。 */
async function submitProfessionalSbFrameImage(sb, slot, poller) {
  const isLast = slot === 'last'
  const prompt = await ensureProfessionalFramePrompt(sb, slot)
  let referenceImages
  if (isLast && lastFrameUseFirstLayoutLock.value) {
    const first = getSbFirstImage(sb.id)
    const firstUrl = first ? (assetImageUrl(first) || first.image_url || first.local_path) : ''
    if (firstUrl) referenceImages = [firstUrl]
  }
  const res = await imagesAPI.create({
    storyboard_id: sb.id,
    drama_id: dramaId.value,
  ...
  if (res?.task_id && typeof poller === 'function') return poller(res.task_id)
  await loadSingleStoryboardMedia(sb.id)
  return { status: 'completed' }
}

function isSingleStoryboardImageGenerating(storyboardId) {
  return generatingSbImageIds.has(storyboardId)
    || generatingSbFirstImageIds.has(storyboardId)
    || generatingSbLastImageIds.has(storyboardId)
}

async function onGenerateSingleStoryboardImage(sb) {
  const shouldGenerateFramePair = isSbFirstLastFrameMode(sb)
  if (shouldGenerateFramePair) {
...
async function doUploadSbImage(sbId, file, slot = 'first') {
  ...
  const useFirstLast = isSbFirstLastFrameMode(sbId)
  const useSlot = useFirstLast ? slot : 'first'
  if (useFirstLast) {
  ...
  try {
    ...
    const uploaded = await imagesAPI.upload({
      ...
      frame_type: useFirstLast ? frameTypeForSlot(useSlot) : undefined,
    })
...
function syncStoryboardStateFromEpisode(ep) {
  ...
  const nextUseFirstLastFrame = {}
  const nextUniversalSegment = {}
  const nextImagePrompt = {}
  const nextPolishedPrompt = {}
  ...
  for (const sb of boards) {
    ...
    nextMovement[sb.id] = normalizeStoryboardMovement(sb.movement)
    ...
    nextUseFirstLastFrame[sb.id] = sb.use_first_last_frame == null ? null : !!sb.use_first_last_frame
    nextUniversalSegment[sb.id] = (sb.universal_segment_text ?? '').toString()
    nextImagePrompt[sb.id] = (sb.image_prompt ?? '').toString()
    nextPolishedPrompt[sb.id] = (sb.polished_prompt ?? '').toString()
    nextVideoPrompt[sb.id] = formatVideoPromptForEdit((sb.video_prompt ?? '').toString())
  }
  ...
  sbUseFirstLastFrame.value = nextUseFirstLastFrame
  sbUniversalSegmentText.value = nextUniversalSegment
  sbImagePrompt.value = nextImagePrompt
  sbPolishedPrompt.value = nextPolishedPrompt
  sbVideoPrompt.value = nextVideoPrompt
}
...
function getMovementLabel(m) {
  return storyboardMovementLabel(m)
}
...
function onSbRemoveCharacter(sbId, charId) {
  const id = Number(charId)
  setSbCharacterIds(sbId, getSbCharacterIds(sbId).filter((x) => Number(x) !== id))
}
...
function propsAvailableToAddToSb(sbId) {
  const all = props.value ?? []
  const cur = new Set(getSbPropIds(sbId).map((x) => Number(x)))
  return all.filter((p) => p && !cur.has(Number(p.id)))
}

function onSbAddPropCommand(sbId, propId) {
  const id = Number(propId)
  if (!Number.isFinite(id)) return
  const cur = [...getSbPropIds(sbId)]
  if (!cur.some((x) => Number(x) === id)) cur.push(id)
  setSbPropIds(sbId, cur)
}

function onSbRemoveProp(sbId, propId) {
  const id = Number(propId)
  setSbPropIds(sbId, getSbPropIds(sbId).filter((x) => Number(x) !== id))
}
...
function onSbSelectSceneCommand(sbId, sceneId) {
  const next = sceneId === '__clear__' ? null : Number(sceneId)
  if (next !== null && !Number.isFinite(next)) return
  sbSceneId.value = { ...sbSceneId.value, [sbId]: next }
  onStoryboardSceneChange(sbId)
}
...
async function onRegenAffectedSbImages(assetKey, affectedBoards) {
  ...
  try {
    for (let i = 0; i < affectedBoards.length; i++) {
      ...
      try {
        const useFirstLast = isSbFirstLastFrameMode(sb)
        const pollRegenTask = (taskId) => new Promise((resolve) => {
        ...
        if (useFirstLast) {
          for (const slot of ['first', 'last']) {
            const pollRes = await submitProfessionalSbFrameImage(sb, slot, pollRegenTask)
            if (pollRes?.status !== 'completed') throw new Error(pollRes?.error || `${slot} frame failed`)
          }
          delete sbSelectedImgId.value[sb.id]
          delete sbSelectedLastImgId.value[sb.id]
        } else {
          ...
          if (res?.task_id) {
            const pollRes = await pollRegenTask(res.task_id)
            if (pollRes?.status !== 'completed') throw new Error(pollRes?.error || '任务失败')
          } else {
            await loadSingleStoryboardMedia(sb.id)
          }
        }
...
function getSbFirstFrameUrl(sb) {
  const img = isSbFirstLastFrameMode(sb) ? getSbFirstImage(sb.id) : getSbImage(sb.id)
...
function sbVideoFirstLastUrls(sb, universal, contiguityFirstFrameUrl) {
  ...
  let last = undefined
  if (isSbFirstLastFrameMode(sb) && !universal) {
...
async function onExportStoryboardSheet() {
  ...
  function resolveFirstFramePrompt(sbId) {
    ...
    if (isSbFirstLastFrameMode(sbId)) return buildFirstFrameImagePrompt(sbId)
    return ''
  }
  ...
  function resolveLastFramePrompt(sbId) {
    ...
    if (imgPrompt) return imgPrompt
    if (isSbFirstLastFrameMode(sbId)) return buildLastFrameImagePrompt(sbId)
    return ''
  }
...
/** 本镜设置优先；旧分镜未保存逐镜值时继承项目全局设置。 */
function isSbFirstLastFrameMode(sbOrId) {
  const sbId = typeof sbOrId === 'object' && sbOrId != null ? sbOrId.id : sbOrId
  if (sbId == null || isSbUniversalMode(sbId)) return false
  const local = sbUseFirstLastFrame.value[sbId]
  return local == null ? !!storyboardUseFirstLastFrame.value : !!local
}

function setSbUseFirstLastFrameId(sbId, enabled) {
  if (sbId == null || isSbUniversalMode(sbId)) return
  sbUseFirstLastFrame.value = { ...sbUseFirstLastFrame.value, [sbId]: !!enabled }
}
...
function setSbCreationModeId(sbId, mode) {
  ...
  if (m === 'universal') {
    sbUseFirstLastFrame.value = { ...sbUseFirstLastFrame.value, [sbId]: false }
  }
}
...
async function onToggleSbUniversalMode(sb) {
  ...
  const previousFirstLast = sbUseFirstLastFrame.value[sb.id]
  sbCreationMode.value = { ...sbCreationMode.value, [sb.id]: next }
  if (next === 'universal') {
    sbUseFirstLastFrame.value = { ...sbUseFirstLastFrame.value, [sb.id]: false }
  }
  try {
    await storyboardsAPI.update(sb.id, {
      creation_mode: next,
      ...(next === 'universal' ? { use_first_last_frame: false } : {}),
    })
    const list = store.currentEpisode?.storyboards
    if (Array.isArray(list)) {
      const row = list.find((x) => Number(x.id) === Number(sb.id))
      if (row) {
        row.creation_mode = next
        if (next === 'universal') row.use_first_last_frame = false
      }
    }
  } catch (e) {
    sbCreationMode.value = { ...sbCreationMode.value, [sb.id]: cur }
    const restoredFirstLast = { ...sbUseFirstLastFrame.value }
    if (previousFirstLast == null) delete restoredFirstLast[sb.id]
    else restoredFirstLast[sb.id] = previousFirstLast
    sbUseFirstLastFrame.value = restoredFirstLast
    ElMessage.error(e.message || '保存失败')
  }
}

async function onToggleSbFirstLastFrame(sb, enabled) {
  if (!sb?.id || isSbUniversalMode(sb.id)) return
  const previousLocal = sbUseFirstLastFrame.value[sb.id]
  const next = !!enabled
  setSbUseFirstLastFrameId(sb.id, next)
  try {
    await storyboardsAPI.update(sb.id, { use_first_last_frame: next })
    const list = store.currentEpisode?.storyboards
    if (Array.isArray(list)) {
      const row = list.find((x) => Number(x.id) === Number(sb.id))
      if (row) row.use_first_last_frame = next
    }
  } catch (e) {
    const restored = { ...sbUseFirstLastFrame.value }
    if (previousLocal == null) delete restored[sb.id]
    else restored[sb.id] = previousLocal
    sbUseFirstLastFrame.value = restored
    ElMessage.error(e.message || '首尾帧设置保存失败')
  }
}
...
async function getActiveVideoAiConfig() {
  ...
  try {
    const rows = await aiAPI.listRuntime('video')
...
async function onSaveSbImagePrompt(sb) {
  if (!sb?.id || savingSbImagePromptIds.has(sb.id)) return
  const normalized = (sbImagePrompt.value[sb.id] || '').toString().trim()
  savingSbImagePromptIds.add(sb.id)
  try {
    await storyboardsAPI.update(sb.id, { image_prompt: normalized || null })
    sbImagePrompt.value = {
      ...sbImagePrompt.value,
      [sb.id]: normalized,
    }
    ...
    if (Array.isArray(list)) {
      const row = list.find((x) => Number(x.id) === Number(sb.id))
      if (row) row.image_prompt = normalized || null
    }
    ElMessage.success('图片提示词已保存')
  } catch (e) {
    ElMessage.error(e.message || '保存失败')
  } finally {
    savingSbImagePromptIds.delete(sb.id)
  }
}

async function onSaveSbVideoPrompt(sb) {
  if (!sb?.id || savingSbVideoPromptIds.has(sb.id)) return
  const normalized = (sbVideoPrompt.value[sb.id] || '').replace(/\s+/g, ' ').trim()
  savingSbVideoPromptIds.add(sb.id)
  try {
    await storyboardsAPI.update(sb.id, { video_prompt: normalized || null })
    sbVideoPrompt.value = {
      ...sbVideoPrompt.value,
      [sb.id]: formatVideoPromptForEdit(normalized),
    }
    const list = store.currentEpisode?.storyboards
    if (Array.isArray(list)) {
      const row = list.find((x) => Number(x.id) === Number(sb.id))
      if (row) row.video_prompt = normalized || null
    }
    ElMessage.success('视频提示词已保存')
  } catch (e) {
    ElMessage.error(e.message || '保存失败')
  } finally {
    savingSbVideoPromptIds.delete(sb.id)
  }
}

async function onSaveSbPolishedPrompt(sb) {
  if (!sb?.id || savingSbPolishedPromptIds.has(sb.id)) return
  const normalized = (sbPolishedPrompt.value[sb.id] || '').toString().trim()
  savingSbPolishedPromptIds.add(sb.id)
  try {
    await storyboardsAPI.update(sb.id, { polished_prompt: normalized || null })
    sbPolishedPrompt.value = {
      ...sbPolishedPrompt.value,
      [sb.id]: normalized,
    }
    const list = store.currentEpisode?.storyboards
    if (Array.isArray(list)) {
      const row = list.find((x) => Number(x.id) === Number(sb.id))
      if (row) row.polished_prompt = normalized || null
    }
    ElMessage.success('图片优化提示词已保存')
  } catch (e) {
    ElMessage.error(e.message || '保存失败')
  } finally {
    savingSbPolishedPromptIds.delete(sb.id)
  }
}
...
function buildStoryboardConfigPayload(sb) {
  ...
  return {
    ...
    atmosphere: (sbAtmosphere.value[id] || '').toString().trim() || null,
    result: (sbResult.value[id] || '').toString().trim() || null,
    angle: (sbAngle.value[id] || '').toString().trim() || null,
    angle_h: sbAngleH.value[id] || null,
    angle_v: sbAngleV.value[id] || null,
    angle_s: sbAngleS.value[id] || null,
    movement: (sbMovement.value[id] || '').toString().trim() || null,
    lighting_style: sbLighting.value[id] || null,
    depth_of_field: sbDof.value[id] || null,
    shot_type: (sbShotType.value[id] || '').toString().trim() || null,
    layout_description: (sbLayoutDescription.value[id] || '').toString().trim() || null,
    creation_mode: sbCreationMode.value[id] === 'universal' ? 'universal' : 'classic',
    use_first_last_frame: isSbFirstLastFrameMode(id),
    universal_segment_text: (sbUniversalSegmentText.value[id] || '').toString().trim() || null,
  }
}

async function onPolishInlineSbPrompt(sb) {
  if (!sb?.id) return
  polishingStoryboardPromptIds.add(sb.id)
  try {
    const res = await storyboardsAPI.polishPrompt(sb.id)
    if (res?.polished_prompt) {
      sbPolishedPrompt.value = { ...sbPolishedPrompt.value, [sb.id]: res.polished_prompt }
      ElMessage.success('图片优化提示词已生成')
    }
  } catch (e) {
    ElMessage.error(e.message || '生成失败，请检查文本模型配置')
  } finally {
    polishingStoryboardPromptIds.delete(sb.id)
  }
}

/** 保存当前分镜配置；经典模式按最新字段同步重建 video_prompt。 */
async function onSaveStoryboardWorkspace(sb, options = {}) {
  if (!sb?.id) return false
  savingStoryboardWorkspaceIds.add(sb.id)
  try {
    const payload = {
      ...buildStoryboardConfigPayload(sb),
      image_prompt: (sbImagePrompt.value[sb.id] || '').toString().trim() || null,
      polished_prompt: (sbPolishedPrompt.value[sb.id] || '').toString().trim() || null,
      video_prompt: (sbVideoPrompt.value[sb.id] || '').replace(/\s+/g, ' ').trim() || null,
    }
    const result = await saveStoryboardWorkspace(storyboardsAPI, sb.id, payload)
    if (result.storyboard?.video_prompt != null) {
      sbVideoPrompt.value = {
        ...sbVideoPrompt.value,
        [sb.id]: formatVideoPromptForEdit(result.storyboard.video_prompt),
      }
    }
    await loadDrama()
    if (!options.silent) {
      ElMessage.success(
        result.rebuiltVideoPrompt
          ? '分镜配置已保存，视频提示词已更新'
          : '全能分镜配置与片段描述已保存'
      )
    }
    ...
    savingStoryboardWorkspaceIds.delete(sb.id)
  }
}

function onOpenVideoParamsDialog(sb) {
  if (!sb?.id) return
  videoParamsPageScrollPosition = {
    left: window.scrollX,
    top: window.scrollY,
  }
  videoParamsTarget.value = sb
  showVideoParamsDialog.value = true
  nextTick(restoreVideoParamsPageScroll)
}

function restoreVideoParamsPageScroll() {
  if (!videoParamsPageScrollPosition || typeof window === 'undefined') return
  const { left, top } = videoParamsPageScrollPosition
  window.scrollTo({ left, top, behavior: 'auto' })
}

/** 取消弹窗时丢弃未保存草稿；保存完成后则从最新剧集数据同步。 */
function onVideoParamsDialogClosed() {
  syncStoryboardStateFromEpisode(store.currentEpisode)
  videoParamsTarget.value = null
  const savedPosition = videoParamsPageScrollPosition
  videoParamsPageScrollPosition = null
...
async function onSaveVideoParams() {
  ...
  try {
    const saved = await onSaveStoryboardWorkspace(sb)
    if (saved) showVideoParamsDialog.value = false
  } finally {
...
async function onSplitSbByAudio(sb) {
  ...
  try {
    const saved = await onSaveStoryboardWorkspace(sb, { silent: true })
    if (!saved) throw new Error('当前分镜保存失败，已取消拆镜')
...
async function onAddSingleStoryboard(){
  ...
  try {
    ...
    await storyboardsAPI.create({
      ...
      title: `镜头 ${maxNum + 1}`,
      description: '',
      creation_mode: storyboardUniversalOmni.value ? 'universal' : 'classic',
      use_first_last_frame: storyboardUniversalOmni.value ? false : !!storyboardUseFirstLastFrame.value,
    })
...
async function startBatchImageGeneration() {
  ...
  try {
    ...
    const todo = boards.filter((sb) =>
      isSbFirstLastFrameMode(sb) ? missingSbFrameSlots(sb).length > 0 : !hasSbImage(sb)
    )
    if (todo.length === 0) {
    ...
    const worker = async () => {
      while (queueIdx < todo.length) {
        ...
        try {
          if (useFirstLast) {
            for (const slot of missingSbFrameSlots(sb)) {
              const pollRes = await submitProfessionalSbFrameImage(
                sb,
                slot,
...
async function runOneClickPipeline(textOnly = false) {
  ...
  try {
    ...
    if (sceneList.length === 0) {
      ...
      try {
        const res = await dramaAPI.extractBackgrounds(episodeId, { model: undefined, style })
        const taskId = res?.task_id
        if (taskId) {
          const result = await pollTaskWithPause(taskId, () => loadDrama())
          if (result?.paused) { await waitForResume(); return }
    ...
    // 步骤 8：生成分镜图
    {
      await loadStoryboardMedia()
      boards = store.storyboards || []
      const boardsWithoutImg = boards.filter((sb) =>
        isSbFirstLastFrameMode(sb) ? missingSbFrameSlots(sb).length > 0 : !hasSbImage(sb)
      )
      const concurrency = pipelineConcurrency.value
      ...
      const { paused } = await runConcurrently(boardsWithoutImg, concurrency, async (sb) => {
        ...
        try {
          ...
          const ok = await pipelineWithRetry(stepName, async () => {
            ...
            if (useFirstLast) {
              for (const slot of missingSbFrameSlots(sb)) {
                const result = await submitProfessionalSbFrameImage(
                  sb,
                  slot,
                  (taskId) => pollTaskWithPause(taskId, () => loadSingleStoryboardMedia(sb.id))
                )
                if (result?.paused) return { paused: true }
                if (result?.error) throw new Error(result.error)
...
html.light .select-script-item {
  border-color: rgba(90, 107, 184, 0.15);
}
...
.film-create {
  ...
  background: #16171e;
  background-image:
    radial-gradient(ellipse 80% 50% at 60% -5%, rgba(90, 107, 184, 0.13) 0%, transparent 65%),
    radial-gradient(ellipse 50% 40% at 90% 50%, rgba(201, 106, 58, 0.07) 0%, transparent 55%),
...
html.light .film-create {
  background: #f8f7ff;
  background-image:
    radial-gradient(ellipse 80% 50% at 10% -10%, rgba(201, 106, 58, 0.08) 0%, transparent 50%),
    radial-gradient(ellipse 50% 40% at 85% 110%, rgba(90, 107, 184, 0.06) 0%, transparent 50%);
  color: #211922;
}
.header {
...
html.light .header {
  ...
  border-bottom-color: rgba(201, 106, 58, 0.1) !important;
  box-shadow: 0 1px 0 rgba(201, 106, 58,0.06), 0 4px 20px rgba(201, 106, 58, 0.05) !important;
}
.header-inner {
  display: flex;
  align-items: center;
...
.logo:hover { filter: drop-shadow(0 0 10px rgba(201, 106, 58, 0.5)); }
.logo-main {
  font-size: 1.05rem;
  font-weight: 700;
...
html.light .logo-main {
  background: linear-gradient(135deg, #a94f27, #435ee5);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  background-clip: text;
}
...
html.light .page-title {
  color: #6b7280;
  background: rgba(90, 107, 184, 0.04);
  border-color: rgba(90, 107, 184, 0.1);
}
...
html.light .btn-theme {
  --el-button-bg-color: rgba(90, 107, 184, 0.04);
  --el-button-border-color: rgba(90, 107, 184, 0.12);
  --el-button-text-color: #6b7280;
  --el-button-hover-bg-color: rgba(90, 107, 184, 0.08);
  --el-button-hover-border-color: rgba(90, 107, 184, 0.3);
  --el-button-hover-text-color: #435ee5;
}
/* ===== 左侧固定侧边栏 ===== */
.quick-nav {
  position: fixed;
...
html.light .quick-nav {
  ...
  border-right-color: rgba(201, 106, 58, 0.1);
  box-shadow: 1px 0 0 rgba(201, 106, 58,0.06), 4px 0 20px rgba(201, 106, 58, 0.04);
}
...
.atp-count-badge {
  font-size: 0.68rem;
  background: rgba(201, 106, 58, 0.25);
  color: #f1c2a8;
  border-radius: 8px;
  padding: 1px 5px;
  min-width: 16px;
...
html.light .atp-title { color: #c96a3a; }
html.light .atp-count-badge { background: rgba(201, 106, 58,0.12); color: #c96a3a; }
html.light .atp-spin-dot { background: #c96a3a; }
html.light .atp-item-dot { background: #df8051; }
...
html.light .atp-panel { border-top-color: rgba(201, 106, 58,0.15); }
.nav-sidebar-header {
  display: flex;
  align-items: center;
...
html.light .nav-sidebar-header { border-bottom-color: rgba(201, 106, 58, 0.12); }
...
html.light .nav-sidebar-title { color: #c96a3a; }
.nav-toggle {
  display: flex;
  align-items: center;
  justify-content: center;
...
html.light .nav-step:hover { background: rgba(90, 107, 184,0.05); }
...
.dot-generating {
  ...
  color: #e8a07a;
  box-shadow: 0 0 8px rgba(201, 106, 58, 0.2);
}
.dot-done {
...
html.light .nav-step:hover .step-label { color: #211922; }
.status-done .step-label { color: #6ee7b7; }
html.light .status-done .step-label { color: #059669; }
.status-generating .step-label { color: #f1c2a8; }
html.light .status-generating .step-label { color: #c96a3a; }
...
html.light .nav-sub-list { background: rgba(90, 107, 184,0.03); }
.nav-sub-item {
  display: flex;
  align-items: center;
...
html.light .nav-sub-item:hover { color: #211922; background: rgba(90, 107, 184,0.06); }
...
html.light .card {
  ...
  border-color: rgba(201, 106, 58, 0.08);
  box-shadow: 0 1px 0 rgba(255,255,255,0.8) inset, 0 4px 20px rgba(90, 107, 184, 0.05);
}
html.light .card:hover {
  border-color: rgba(201, 106, 58, 0.18);
  box-shadow: 0 1px 0 rgba(255,255,255,0.8) inset, 0 8px 36px rgba(90, 107, 184, 0.08);
}
.section-title {
  font-size: 1.05rem;
  margin: 0 0 4px;
...
html.light .section-title { color: #211922; }
.pipeline-section {
...
.asset-storyboard-link {
  ...
  margin-top: 8px;
  padding: 6px 8px;
  background: rgba(90, 107, 184, 0.07);
  border: 1px solid rgba(90, 107, 184, 0.18);
  border-radius: 6px;
  min-height: 28px;
}
.asl-label {
...
.asl-chip:hover {
  background: rgba(90, 107, 184, 0.28);
  box-shadow: 0 0 6px rgba(90, 107, 184, 0.4);
  color: #c7cee8;
}
.asl-regen-btn {
...
.resource-batch-actions :deep(.el-button) {
  margin-left: 0;
}
.resource-batch-progress {
  color: #a1a1aa;
  font-size: 12px;
  white-space: nowrap;
}
...
.image-preview-nav:hover {
  background: rgba(201, 106, 58, 0.88);
...
html.light .asset-cover-actions { border-top-color: rgba(201, 106, 58,0.1); }
/* 额外参考图缩略图条 */
.extra-images-strip {
  display: flex;
  flex-wrap: wrap;
...
.extra-thumb:hover { border-color: #e8a07a; }
...
html.light .extra-images-strip { background: rgba(201, 106, 58,0.05); }
.empty-tip {
  color: #5a5a66;
  font-size: 0.9rem;
  padding: 16px 0;
...
html.light .asset-item {
  ...
  border: 1px solid rgba(201, 106, 58, 0.12);
  box-shadow: 0 2px 10px rgba(201, 106, 58, 0.06);
}
html.light .asset-item:hover {
  box-shadow: 0 6px 20px rgba(201, 106, 58, 0.12);
...
.segment-header-inner {
  ...
  gap: 10px;
  padding: 10px 18px;
  background: linear-gradient(90deg, rgba(201, 106, 58,0.12) 0%, transparent 80%);
  border-left: 3px solid rgba(201, 106, 58,0.6);
...
html.light .segment-header-inner {
  background: linear-gradient(90deg, rgba(201, 106, 58,0.07) 0%, transparent 80%);
  border-left-color: rgba(201, 106, 58,0.5);
}
html.light .segment-title-text { color: #211922; }
html.light .segment-index-badge { color: #c96a3a; background: rgba(201, 106, 58,0.08); }
...
html.light .storyboard-row {
  ...
  border-color: rgba(201, 106, 58, 0.06);
  box-shadow: 0 1px 0 rgba(255,255,255,0.7) inset, 0 2px 12px rgba(90, 107, 184, 0.04);
}
html.light .storyboard-row:hover {
  border-color: rgba(201, 106, 58, 0.18);
  box-shadow: 0 1px 0 rgba(255,255,255,0.7) inset, 0 6px 24px rgba(90, 107, 184, 0.08);
  transform: translateY(-1px);
}
...
.sb-ctrl-config-btn.el-button {
  border-color: rgba(201, 106, 58,0.45);
  color: #e8a07a;
...
.sb-ctrl-config-btn.el-button:hover {
  border-color: #df8051;
  color: #fff;
  background: rgba(201, 106, 58,0.6);
}
...
.sb-ctrl-collapse.el-button {
  margin-left: auto;
  color: #a1a1aa;
}
.sb-ctrl-collapse .el-icon {
  margin-left: 3px;
}
.sb-workspace-editor {
  margin: 0 0 10px;
  ...
  border-radius: 14px;
  background:
    radial-gradient(circle at 100% 0, rgba(201, 106, 58, 0.12), transparent 34%),
    linear-gradient(180deg, rgba(31, 32, 43, 0.98), rgba(24, 24, 32, 0.98));
  box-shadow: 0 12px 32px rgba(0, 0, 0, 0.18);
  overflow: hidden;
}
html.light .sb-workspace-editor {
  border-color: rgba(201, 106, 58, 0.16);
  background:
    radial-gradient(circle at 100% 0, rgba(201, 106, 58, 0.08), transparent 34%),
    linear-gradient(180deg, #ffffff, #faf9ff);
  box-shadow: 0 10px 28px rgba(79, 70, 229, 0.06);
}
.sb-workspace-editor__header {
  min-height: 72px;
  margin: 0 -18px 18px;
  padding: 14px 18px;
  display: flex;
...
html.light .sb-workspace-editor__header {
  border-bottom-color: rgba(201, 106, 58, 0.1);
  background: rgba(201, 106, 58, 0.025);
}
.sb-workspace-editor__identity {
  min-width: 0;
  display: flex;
  align-items: center;
  gap: 11px;
}
.sb-workspace-editor__shot-no {
  min-width: 54px;
  height: 28px;
  padding: 0 9px;
  display: inline-flex;
  align-items: center;
  justify-content: center;
  border: 1px solid rgba(201, 106, 58, 0.32);
  border-radius: 8px;
...
.sb-workspace-editor__header h3 {
  margin: 0;
  color: #f4f4f5;
  font-size: 16px;
  line-height: 1.2;
}
html.light .sb-workspace-editor__header h3 {
  color: #18181b;
}
.sb-workspace-editor__identity p {
  max-width: 28em;
  margin: 3px 0 0;
  overflow: hidden;
  color: #71717a;
...
html.light .sb-workspace-editor__shot-no {
  border-color: rgba(201, 106, 58, 0.2);
  background: rgba(201, 106, 58, 0.07);
  color: #c96a3a;
}
html.light .sb-workspace-editor__identity p {
  color: #6b7280;
}
.sb-workspace-editor__header-tools {
  display: flex;
  align-items: center;
  justify-content: flex-end;
  gap: 16px;
}
.sb-config-scope-hint {
  color: #e8a07a;
  font-size: 12px;
  font-weight: 600;
}
html.light .sb-config-scope-hint {
  color: #c96a3a;
}
.sb-editor-mode-control {
  display: flex;
  align-items: center;
  gap: 8px;
}
.sb-editor-mode-control > span {
  color: #a1a1aa;
  font-size: 11px;
  font-weight: 600;
  white-space: nowrap;
}
html.light .sb-editor-mode-control > span {
  color: #52525b;
}
.sb-editor-first-last-control {
  margin-right: 0;
  white-space: nowrap;
}
.sb-workspace-editor__header-tools > .el-button {
  min-width: 112px;
  margin-left: 0;
}
.sb-editor-section-heading {
  margin-bottom: 10px;
  display: flex;
  align-items: center;
  justify-content: space-between;
...
.sb-editor-section-heading > div {
  display: flex;
  align-items: center;
  gap: 7px;
}
.sb-editor-section-heading strong {
  color: #f4f4f5;
  font-size: 13px;
}
.sb-editor-section-heading > span {
  color: #71717a;
  font-size: 10px;
}
html.light .sb-editor-section-heading strong {
  color: #27272a;
}
.sb-editor-basic-grid {
  display: grid;
  grid-template-columns: repeat(6, minmax(0, 1fr));
  gap: 10px;
  align-items: end;
}
.sb-editor-field--title,
.sb-editor-field--location,
.sb-editor-field--movement {
  grid-column: span 2;
}
.sb-editor-field--atmosphere {
  grid-column: span 3;
}
.sb-editor-field {
  min-width: 0;
...
.sb-editor-field > span {
  color: #a1a1aa;
  font-size: 11px;
  font-weight: 600;
  letter-spacing: 0.02em;
}
html.light .sb-editor-field > span {
  color: #52525b;
}
.sb-editor-field :deep(.el-input),
.sb-editor-field :deep(.el-select),
.sb-editor-field :deep(.el-input-number) {
  width: 100%;
}
.sb-editor-advanced {
  margin-top: 14px;
...
html.light .sb-editor-advanced {
  border-color: rgba(201, 106, 58, 0.1);
  background: rgba(201, 106, 58, 0.025);
}
.sb-editor-advanced summary {
  min-height: 40px;
  padding: 0 12px;
  display: flex;
  align-items: center;
...
.sb-editor-advanced summary::-webkit-details-marker {
  display: none;
}
.sb-editor-advanced summary::before {
  content: '›';
  color: #e8a07a;
  font-size: 18px;
  line-height: 1;
...
.sb-editor-advanced[open] summary::before {
  transform: rotate(90deg);
}
.sb-editor-advanced summary > span {
  font-size: 12px;
  font-weight: 650;
}
.sb-editor-advanced summary > small {
  color: #71717a;
  font-size: 11px;
}
html.light .sb-editor-advanced summary {
  color: #3f3f46;
}
html.light .sb-editor-advanced summary > small {
  color: #71717a;
}
.sb-editor-advanced__grid {
  padding: 12px;
  display: grid;
  grid-template-columns: minmax(300px, 1.5fr) minmax(150px, 0.75fr) minmax(150px, 0.75fr);
  gap: 12px;
  border-top: 1px solid rgba(255, 255, 255, 0.06);
}
html.light .sb-editor-advanced__grid {
  border-top-color: rgba(201, 106, 58, 0.08);
}
.sb-editor-angle-row {
  display: grid;
...
.sb-editor-field--layout {
  grid-column: 1 / -1;
}
.sb-editor-layout-row {
  display: flex;
  align-items: flex-start;
  gap: 8px;
}
.sb-editor-layout-row :deep(.el-textarea) {
  flex: 1;
}
.sb-editor-content-grid {
  margin-top: 16px;
  display: grid;
...
.sb-editor-card {
  ...
  border-radius: 11px;
  background: rgba(9, 9, 13, 0.18);
}
html.light .sb-editor-card {
  border-color: rgba(201, 106, 58, 0.1);
  background: rgba(255, 255, 255, 0.74);
}
.sb-editor-card--prompts {
  grid-column: 1 / -1;
  display: flex;
  flex-direction: column;
  box-shadow:
    inset 0 2px 0 rgba(59, 130, 246, 0.52),
    inset 0 -2px 0 rgba(249, 115, 22, 0.36);
}
.sb-editor-card--script {
  box-shadow: inset 0 2px 0 rgba(201, 106, 58, 0.55);
}
.sb-editor-card__title {
  min-height: 32px;
  margin-bottom: 12px;
  display: flex;
  align-items: flex-start;
...
.sb-editor-card__title > div {
  display: flex;
  align-items: center;
  gap: 7px;
}
.sb-editor-card__title strong {
  color: #f4f4f5;
  font-size: 13px;
}
.sb-editor-card__title > span {
  color: #71717a;
  font-size: 10px;
  line-height: 1.4;
  text-align: right;
}
html.light .sb-editor-card__title strong {
  color: #27272a;
}
.sb-editor-card__step {
  color: #f1c2a8;
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
  font-size: 10px;
  font-weight: 700;
}
html.light .sb-editor-card__step {
  color: #c96a3a;
}
.sb-editor-script-grid {
  display: grid;
  grid-template-columns: 1fr 1fr;
  gap: 10px;
}
.sb-editor-prompt-workspace {
  display: grid;
  grid-template-columns: minmax(0, 1fr) auto minmax(0, 1fr) minmax(0, 1fr);
  gap: 12px;
  flex: 1;
}
.sb-editor-prompt-pane {
  min-width: 0;
  display: flex;
  flex-direction: column;
  gap: 6px;
}
.sb-editor-prompt-transform {
  display: flex;
  align-items: center;
  justify-content: center;
  padding-top: 22px;
}
.sb-editor-prompt-transform .el-button {
  min-width: 112px;
  margin: 0;
}
.sb-editor-prompt-transform__arrow {
  margin-left: 2px;
}
.sb-editor-prompt-textarea {
  width: 100%;
}
.sb-editor-prompt-textarea :deep(.el-textarea__inner) {
  height: 226px;
  min-height: 226px !important;
  resize: none;
}
.sb-editor-video-prompt-footer {
  margin-top: 9px;
  display: flex;
  align-items: center;
  justify-content: space-between;
...
.sb-editor-video-prompt-footer > span {
  color: #71717a;
  font-size: 10px;
  line-height: 1.35;
}
...
html.light .sb-editor-audio-tip {
  color: #92400e;
  background: #fffbeb;
}
.sb-media-panel-heading {
  margin-bottom: 10px;
  display: flex;
...
.sb-media-panel-heading strong {
  color: #e4e4e7;
  font-size: 12px;
}
.sb-media-panel-heading span {
  color: #71717a;
  font-size: 10px;
  text-align: right;
}
html.light .sb-media-panel-heading strong {
  color: #27272a;
}
...
@media (max-width: 1180px) {
  ...
  .storyboard-row .sb-panel {
    width: 100%;
    border-right: none;
    border-bottom: 1px solid rgba(255, 255, 255, 0.05);
  }
  .storyboard-row .sb-panel:last-child {
    border-bottom: none;
  }
  html.light .storyboard-row .sb-panel {
    border-bottom-color: rgba(201, 106, 58, 0.08);
  }
}
...
html.light .sb-panel {
  border-right-color: rgba(201, 106, 58,0.08);
}
...
html.light .sb-selected-thumbs {
  border-color: rgba(201, 106, 58, 0.2);
  background: linear-gradient(145deg, #faf9f6 0%, #f7ebe4 100%);
}
.sb-thumb-row {
  display: block;
  margin-bottom: 14px;
...
.sb-thumb-item {
  ...
  border-radius: 9px;
  overflow: hidden;
  background: rgba(9, 9, 13, 0.22);
  transition: border-color 0.18s, transform 0.18s, box-shadow 0.18s;
}
.sb-thumb-item.sb-thumb-clickable {
  cursor: pointer;
}
.sb-thumb-item.sb-thumb-clickable:hover {
  border-color: rgba(201, 106, 58, 0.65);
  box-shadow: 0 8px 18px rgba(0, 0, 0, 0.2);
  transform: translateY(-1px);
}
html.light .sb-thumb-item {
  border-color: rgba(201, 106, 58, 0.12);
...
html.light .sb-thumb-add {
  border-color: #d4d4d8;
  color: #71717a;
}
html.light .sb-thumb-add:hover {
  color: #18181b;
  border-color: #a1a1aa;
  background: #f4f4f5;
}
...
.sb-fl-first-lock-opt--overlay :deep(.el-checkbox__input) {
  flex: none;
}
.sb-fl-first-lock-opt--overlay :deep(.el-checkbox__label) {
  padding-left: 0;
  color: #f4f4f5;
  line-height: 1.15;
  text-align: center;
...
.sb-fl-first-lock-text {
  display: block;
}
html.light .sb-fl-first-lock-opt--overlay {
  border-color: rgba(201, 106, 58, 0.24);
  background: rgba(255, 255, 255, 0.9);
}
html.light .sb-fl-first-lock-opt--overlay :deep(.el-checkbox__label) {
  color: #3f3f46;
  text-shadow: none;
}
.sb-fl-slot-prompt {
...
html.light .sb-frame-history {
  border-color: rgba(201, 106, 58, 0.12);
  background: rgba(255, 255, 255, 0.64);
}
.sb-frame-history--first {
  box-shadow: inset 0 2px 0 rgba(59, 130, 246, 0.5);
}
.sb-frame-history--last {
  box-shadow: inset 0 2px 0 rgba(249, 115, 22, 0.5);
}
.sb-frame-history__header {
  min-height: 26px;
  margin-bottom: 7px;
  display: flex;
  align-items: center;
...
.sb-frame-history__header > div {
  min-width: 0;
  display: flex;
  align-items: baseline;
  gap: 6px;
}
.sb-frame-history__header strong {
  color: #e4e4e7;
  font-size: 11px;
  white-space: nowrap;
}
.sb-frame-history__header span {
  color: #71717a;
  font-size: 10px;
  white-space: nowrap;
}
html.light .sb-frame-history__header strong {
  color: #27272a;
}
.sb-frame-history__nav {
  display: flex;
  align-items: center;
  gap: 4px;
}
.sb-frame-history__nav button {
  margin: 0;
  padding: 0;
  display: inline-flex;
  align-items: center;
...
.sb-frame-history__nav button:hover {
  border-color: rgba(201, 106, 58, 0.55);
  background: rgba(201, 106, 58, 0.2);
  color: #fff;
}
html.light .sb-frame-history__nav button {
  border-color: rgba(201, 106, 58, 0.16);
  background: rgba(201, 106, 58, 0.045);
  color: #52525b;
}
.sb-frame-history__track {
  display: flex;
  align-items: stretch;
  gap: 8px;
  width: 100%;
  ...
  scrollbar-color: rgba(201, 106, 58, 0.45) transparent;
}
.sb-frame-history__track::-webkit-scrollbar {
  height: 6px;
}
.sb-frame-history__track::-webkit-scrollbar-thumb {
  border-radius: 999px;
  background: rgba(201, 106, 58, 0.45);
}
.sb-frame-history-card {
  position: relative;
  width: 116px;
  min-width: 116px;
  height: 104px;
...
.sb-frame-history-card:hover {
  border-color: rgba(201, 106, 58, 0.72);
  box-shadow: 0 8px 20px rgba(0, 0, 0, 0.24);
  transform: translateY(-1px);
}
.sb-frame-history-card.is-current {
  border-color: #df8051;
  box-shadow: 0 0 0 2px rgba(201, 106, 58, 0.18);
}
...
.sb-frame-history-card:focus-visible {
  border-color: #e8a07a;
  box-shadow: 0 0 0 2px rgba(201, 106, 58, 0.28);
}
.sb-frame-history-card:hover .extra-thumb-remove,
.sb-frame-history-card:hover .thumb-preview-btn,
.sb-frame-history-card:focus-within .extra-thumb-remove,
.sb-frame-history-card:focus-within .thumb-preview-btn {
  opacity: 1;
}
html.light .sb-frame-history-card {
  background: #fff;
}
.sb-frame-history-card__preview {
  width: 100%;
  height: 100%;
  overflow: hidden;
  background: rgba(0, 0, 0, 0.18);
}
.sb-frame-history-card__preview img {
  width: 100%;
  height: 100%;
  display: block;
  object-fit: cover;
}
.sb-frame-history-card__label {
  position: absolute;
  right: 0;
  bottom: 0;
  left: 0;
...
html.light .sb-frame-history__empty {
  border-color: rgba(201, 106, 58, 0.12);
  color: #9ca3af;
}
...
html.light .sb-media-empty-text {
  color: #c96a3a;
}
html.light .sb-image-history {
  border-color: rgba(201, 106, 58, 0.12);
  background: rgba(246, 246, 243, 0.58);
}
.sb-image-history__header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 8px;
...
.sb-image-history__header span {
  color: #71717a;
  font-size: 0.72rem;
}
html.light .sb-image-history__header {
  color: #3f3f46;
}
.sb-image-history__empty {
  min-height: 58px;
  display: flex;
  align-items: center;
  justify-content: center;
...
html.light .sb-image-history__empty {
  border-color: rgba(201, 106, 58, 0.12);
}
.sb-imgs-strip {
  display: flex;
...
.sb-img-thumb.is-current,
.sb-video-thumb.is-current {
  border-color: #df8051;
  box-shadow: 0 0 0 2px rgba(201, 106, 58, 0.18);
}
.sb-img-thumb img {
...
html.light .sb-video-history {
  border-color: rgba(201, 106, 58, 0.12);
  background: rgba(255, 255, 255, 0.64);
}
.sb-video-history__header {
  margin-bottom: 7px;
  display: flex;
  align-items: center;
  justify-content: space-between;
...
.sb-video-history__header strong {
  color: #e4e4e7;
  font-size: 11px;
}
.sb-video-history__header span {
  color: #71717a;
  font-size: 10px;
  white-space: nowrap;
}
html.light .sb-video-history__header strong {
  color: #27272a;
}
.sb-video-history__empty {
  min-height: 58px;
  display: flex;
  align-items: center;
  justify-content: center;
...
html.light .sb-video-history__empty {
  border-color: rgba(201, 106, 58, 0.12);
}
.sb-videos-strip {
  display: flex;
...
.sb-videos-strip::-webkit-scrollbar {
  height: 6px;
}
.sb-videos-strip::-webkit-scrollbar-thumb {
  border-radius: 999px;
  background: rgba(201, 106, 58, 0.45);
}
.sb-video-thumb {
...
.sb-video-params-bar .sb-video-prompt-inline-input {
  flex: 1;
  min-width: 0;
}
.sb-video-params-bar > .el-button {
  flex-shrink: 0;
}
.sb-video-prompt-inline-input :deep(.el-textarea__inner) {
...
.sb-polished-prompt-section {
  margin-top: 10px;
  flex-shrink: 0;
}
.sb-polished-prompt-label,
.sb-polished-prompt-label > div {
  display: flex;
  align-items: center;
}
.sb-polished-prompt-label {
...
html.light .sb-polished-prompt-label > div {
  color: #3f3f46;
}
.sb-polished-prompt-bar {
  display: flex;
  align-items: center;
  gap: 8px;
}
.sb-polished-prompt-input {
  min-width: 0;
  flex: 1;
}
.sb-polished-prompt-input :deep(.el-textarea__inner) {
  height: 180px !important;
  min-height: 180px !important;
  max-height: 180px !important;
  resize: none;
...
.sb-media-prompt-section {
  min-height: 220px;
  overflow: hidden;
}
.sb-video-prompt-section {
  flex-shrink: 0;
}
.sb-media-actions {
  min-height: 38px;
}
.sb-polished-prompt-bar > .el-button {
  flex-shrink: 0;
}
.sb-video-prompt-row {
  display: flex;
...
.vp-dialog-layout {
  --vp-config-column-height: clamp(420px, calc(100vh - 220px), 720px);
  display: grid;
  grid-template-columns: minmax(0, 1fr) minmax(320px, 0.48fr);
  gap: 20px;
  align-items: start;
}
:global(.storyboard-config-dialog__body) {
...
.vp-params-column,
.vp-video-prompt-column {
  min-width: 0;
  height: var(--vp-config-column-height);
...
.vp-params-column :deep(.el-divider) {
  margin: 18px 0;
}
.vp-params-column :deep(.el-divider__text) {
  color: var(--el-text-color-primary);
  font-size: 13px;
  font-weight: 650;
  background: var(--el-fill-color-lighter);
...
.vp-content-grid .el-form-item {
  min-height: 0;
  margin-bottom: 0;
}
.vp-content-grid :deep(.el-form-item__content),
.vp-content-grid :deep(.el-textarea),
.vp-content-grid :deep(.el-textarea__inner) {
  height: 100%;
  min-height: 0;
}
.vp-content-grid :deep(.el-textarea__inner) {
...
.vp-video-prompt-heading strong {
  color: var(--el-text-color-primary);
  font-size: 14px;
}
.vp-video-prompt-heading span {
  color: var(--el-text-color-secondary);
  font-size: 12px;
}
...
.sb-image-prompt-inline {
  align-items: center;
}
.sb-image-prompt-inline-input {
  min-width: 0;
  flex: 1;
}
.sb-image-prompt-inline-input :deep(.el-textarea__inner) {
  height: 180px !important;
  min-height: 180px !important;
  max-height: 180px !important;
  resize: none;
...
.sb-image-prompt-inline > .el-button {
  flex-shrink: 0;
}
...
.sb-generating-tip {
  ...
  margin-top: 10px;
  background: rgba(201, 106, 58, 0.08);
  border: 1px dashed rgba(201, 106, 58, 0.35);
  border-radius: 10px;
  color: #e8a07a;
  font-size: 0.9rem;
}
...
.sb-export-srt-btn.el-button--primary.is-plain {
  --el-button-bg-color: rgba(201, 106, 58, 0.75);
  --el-button-border-color: #e8a07a;
  --el-button-text-color: #fff;
  --el-button-hover-text-color: #fff;
  --el-button-hover-bg-color: #df8051;
  --el-button-hover-border-color: #f1c2a8;
}
html.light .sb-export-srt-btn.el-button--primary.is-plain {
  --el-button-bg-color: #c96a3a;
  --el-button-border-color: #a94f27;
  --el-button-text-color: #fff;
  --el-button-hover-text-color: #fff;
  --el-button-hover-bg-color: #a94f27;
  --el-button-hover-border-color: #873c1c;
}
...
html.light .sb-narration-input :deep(.el-textarea__inner) {
  ...
  border-color: rgba(201, 106, 58, 0.22) !important;
}
...
.library-scope-tip strong { color: var(--el-text-color-primary); }
...
.sb-ctrl-first-last.el-checkbox {
  height: 22px;
  margin-right: 0;
  padding: 0 7px;
...
.sb-ctrl-first-last.el-checkbox :deep(.el-checkbox__label) {
  padding-left: 5px;
  color: #fbbf24;
  font-size: 11px;
}
.sb-ctrl-first-last.el-checkbox.is-disabled {
  opacity: 0.5;
}
...
html.light .sb-ctrl-first-last.el-checkbox {
  border-color: rgba(217, 119, 6, 0.35);
  background: rgba(245, 158, 11, 0.08);
}
html.light .sb-ctrl-first-last.el-checkbox :deep(.el-checkbox__label) {
  color: #b45309;
}
.vp-angle-selects {
  width: 100%;
...
.vp-layout-row .el-textarea {
  flex: 1;
}
.vp-video-prompt-preview {
  width: 100%;
  min-height: 0;
  flex: 1;
  display: flex;
...
.vp-video-prompt-preview .vp-video-prompt-hint {
  margin-bottom: 7px;
}
.vp-video-prompt-textarea {
  min-height: 0;
  flex: 1;
}
.vp-video-prompt-textarea :deep(.el-textarea__inner) {
  height: 100%;
...
@media (min-width: 901px) {
  ...
  .sb-panel.sb-media-column--fixed {
    height: 864px;
    min-height: 864px;
    max-height: 864px;
    overflow: hidden;
  }
  .sb-panel.sb-media-column--aligned {
    display: grid;
    grid-template-rows: 400px 160px 38px 220px;
    gap: 6px;
    align-content: start;
  }
  .sb-panel.sb-image--universal.sb-media-column--fixed {
    display: flex;
    justify-content: flex-start;
  }
  .sb-panel.sb-image--universal.sb-media-column--fixed .sb-universal-textarea {
    width: 100%;
    flex: 1;
    overflow: hidden;
  }
  .sb-panel.sb-image--universal.sb-media-column--fixed .sb-universal-textarea :deep(.el-textarea__inner) {
    height: 100% !important;
    max-height: none !important;
  }
  .sb-panel.sb-media-column--first-last {
    display: grid;
    grid-template-rows: minmax(0, 1fr) 38px;
    gap: 6px;
    align-content: stretch;
  }
  .sb-media-column--first-last > .sb-image-area {
    width: 100%;
    height: 100%;
    min-height: 0;
    max-height: none;
  ...
  .sb-media-column--first-last > .sb-media-actions {
    width: 100%;
    height: 38px;
    min-height: 38px;
    margin: 0;
  ...
  .sb-media-column--first-last .sb-fl-dual {
    display: grid;
    grid-template-columns: minmax(0, 1fr) 26px minmax(0, 1fr);
    grid-template-rows: 16px minmax(210px, 1fr) 36px 66px 154px 154px;
    gap: 6px 8px;
    min-height: 0;
  }
  .sb-media-column--first-last .sb-fl-slot {
    display: contents;
  }
  .sb-media-column--first-last .sb-fl-slot:first-child > .sb-fl-slot-label {
    grid-column: 1;
    grid-row: 1;
  }
  .sb-media-column--first-last .sb-fl-slot:first-child > .sb-fl-slot-body {
    grid-column: 1;
    grid-row: 2;
  }
  .sb-media-column--first-last .sb-fl-slot:first-child > .sb-fl-slot-prompt {
    grid-column: 1;
    grid-row: 3;
  }
  .sb-media-column--first-last .sb-fl-slot:first-child > .sb-fl-slot-actions {
    grid-column: 1;
    grid-row: 4;
  }
  .sb-media-column--first-last .sb-fl-slot:first-child > .sb-frame-history {
    grid-column: 1 / 4;
    grid-row: 5;
  }
  .sb-media-column--first-last .sb-fl-slot:last-child > .sb-fl-slot-label {
    grid-column: 3;
    grid-row: 1;
  }
  .sb-media-column--first-last .sb-fl-slot:last-child > .sb-fl-slot-body {
    grid-column: 3;
    grid-row: 2;
  }
  .sb-media-column--first-last .sb-fl-slot:last-child > .sb-fl-slot-prompt {
    grid-column: 3;
    grid-row: 3;
  }
  .sb-media-column--first-last .sb-fl-slot:last-child > .sb-fl-slot-actions {
    grid-column: 3;
    grid-row: 4;
  }
  .sb-media-column--first-last .sb-fl-slot:last-child > .sb-frame-history {
    grid-column: 1 / 4;
    grid-row: 6;
  }
  .sb-media-column--first-last .sb-fl-arrow {
    grid-column: 2;
    grid-row: 2;
    align-self: center;
  }
  .sb-media-column--first-last .sb-fl-slot-body {
    min-height: 0;
  }
  .sb-media-column--first-last .sb-fl-slot-body .sb-generated-img {
    max-height: 200px;
  }
  .sb-media-column--first-last .sb-fl-slot-actions {
    min-height: 0;
    overflow: hidden;
  }
  .sb-media-column--first-last .sb-frame-history {
    min-height: 0;
    margin: 0;
    padding: 6px 8px;
    overflow: hidden;
  }
  .sb-media-column--first-last .sb-frame-history__header {
    min-height: 22px;
    margin-bottom: 4px;
  }
  .sb-media-column--aligned > .sb-image-area,
  .sb-media-column--aligned > .sb-video-area {
    width: 100%;
    height: 400px;
    min-height: 400px;
    max-height: 400px;
  ...
  .sb-media-column--aligned > .sb-media-history {
    width: 100%;
    height: 160px;
    min-height: 160px;
    margin: 0;
  ...
  .sb-media-column--aligned > .sb-media-actions {
    width: 100%;
    height: 38px;
    min-height: 38px;
    margin: 0;
  ...
  .sb-media-column--aligned > .sb-media-prompt-section {
    width: 100%;
    height: 220px;
    min-height: 220px;
    margin: 0;
  }
  .sb-media-column--aligned .sb-main-image-wrap {
    width: 100%;
    height: 100%;
    min-height: 0;
  }
  .sb-media-column--aligned .sb-video-thumb {
    width: 144px;
    min-width: 144px;
    height: 104px;
  }
  .sb-media-column--aligned .sb-img-thumb {
    width: 104px;
    min-width: 104px;
    height: 104px;
  }
  .sb-media-column--aligned .sb-videos-strip {
    padding-bottom: 0;
  }
  .sb-media-column--aligned .sb-polished-prompt-label,
  .sb-media-column--aligned .sb-video-prompt-label {
    height: 28px;
    min-height: 28px;
    margin-bottom: 6px;
  }
...
@media (min-width: 901px) and (min-height: 900px) {
  .vp-content-grid {
    ...
    grid-template-rows: repeat(4, minmax(60px, 76px));
    align-content: start;
  }
}
</style>
